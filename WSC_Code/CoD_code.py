"""A Python script to generate a plot that shows how many unique design points are.
...
def ASTRO_DF_refined_design_pts(dimension: int) -> list[tuple[int, int]]:  # noqa: N802
    """Returns a list of total design points sampled per iteration at each dimension.

    in the refined ASTRO-DF that samples 2n+2 design points per iteration but some
    iterations
    sample 2n design points (when reuse is possible).
    """
    iterations = list(range(1, 101))

    def best_sum_design_pts_iter(d):  # noqa: ANN001, ANN202
        return 2 * d + 2 + sum([2 * d for _ in iterations[1:]])

    def worst_sum_design_pts_iter(d):  # noqa: ANN001, ANN202
        return sum([2 * d + 2 for _ in iterations])
    ...
    return [(a, b) for a, b in zip(worst_case, best_case, strict=False)]


def STORM_design_pts(dimension: int) -> list[int]:  # noqa: N802
    ...
    def sum_design_pts_iter(d):  # noqa: ANN001, ANN202
        return (d**2 + 3 * d + 2) / 2 + sum([1 for _ in iterations[1:]])

    return [sum_design_pts_iter(n) for n in range(1, dimension + 1)]


def Powell_design_pts(dimension: int) -> list[int]:  # noqa: N802
    """Returns a list of total design points sampled per-iteration for each dimension.
...
def ASTRO_DF_design_pts(dimension: int) -> list[int]:  # noqa: N802
    """Returns a list of total design points sampled per-iteration for each dimension.
    ...
    def sum_design_pts_iter(d):  # noqa: ANN001, ANN202
        return sum([(d**2 + 3 * d + 2) / 2 for _ in iterations])

    return [sum_design_pts_iter(n) for n in range(1, dimension + 1)]


def STRONG_design_pts(dimension: int) -> list[int]:  # noqa: N802
    """Returns a list of total design points sampled per-iteration for each dimension.
    ...
    def sum_design_pts_iter(d):  # noqa: ANN001, ANN202
        return sum([d * (d + 1) / 2 for _ in iterations])

    return [sum_design_pts_iter(n) for n in range(1, dimension + 1)]


def plot_unique_design_points() -> None:
    """Plots the cumulative unique design points sampled for each algorithm over the.
    ...
    Powell_design_pts(largest_dimension)

r"""Python script to show how the number of samples taken at a design point is decided throughout the run of an algorithm.  # noqa: E501.

There are three plots:
1. The first plot shows how the standard error of a solver changes throughout the run of
the algorithm on ASTRODF
2. The second plot shows the amount of simulation budget used per iteration for a solver
using adaptive sampling as opposed to fixed sampling
3. The third plot shows how the value of
$\frac{\lambda_k\hat{\sigma}^2(\mathbf{x},t)}{\kappa^2\Delta_k^2}$ changes as the monte
carlo
effort (t) increases at a design point $\mathbf{x}$ within an iteration k of the
algorithm.
"""
...
def densify_by_total(points, n_add):  # noqa: ANN001, ANN201
    """Add a given total number of intermediate tuples along a piecewise-linear.

    trajectory.

    Parameters:
        points (list[tuple]): Original list of coordinate tuples.
        n_add (int): Total number of new points to insert between the first and last
        tuple.
    ...
    for i in range(len(points) - 1):
        ...
        for j in range(1, k + 1):
            t = j / (k + 1)
            interp = tuple(p0[d] + t * (p1[d] - p0[d]) for d in range(len(p0)))
            out.append(interp)
...
########################################
def plot_standard_error_multiple_problems(problem_names: list[str]) -> None:
    """Generates a plot showing the standard error of solutions over the run of an.

    algorithm for multiple problems.
    """
    solver_name = "ADAM"
    ...
    for idx, problem_name in enumerate(problem_names):
        ...
        plt.plot(
            iteration,
            std_error,
            label=f"Standard Error over Iterations for {problem_name}",
            color=color(idx),
        )
    plt.xlabel("Iteration")
    plt.ylabel("Standard Error")
    plt.title(
        f"Standard Error of Solutions over Iterations for {solver_name} on {problem_name}"
    )
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def plot_simulation_budget_multiple_problems(problem_names: list[str]) -> None:
    """Generates a plot showing the simulation budget used per iteration for multiple.
    ...
    for idx, problem_name in enumerate(problem_names):
        ...
        plt.plot(
            ...
            budget,
            linestyle="--",
            label=f"ASTRO-DF on {problem_name}",
            color=color(idx),
        )
    ...
    for idx, problem_name in enumerate(problem_names):
        ...
        plt.plot(
            ...
            budget,
            linestyle=":",
            label=f"Trust-Region on {problem_name}",
            color=color(new_idx),
        )
    ...
    plt.plot(
        avg_iterations_astro,
        trend_line_astro(avg_iterations_astro),
        color="black",
        label="ASTRO-DF Trend Line",
    )
    plt.plot(
        avg_iterations_trust,
        trend_line_trust(avg_iterations_trust),
        color="grey",
        label="Trust-Region Trend Line",
    )

    plt.xlabel("Iteration")
    plt.ylabel("Simulation Budget Used")
    plt.title("Simulation Budget per Iteration")
...
def plot_simulation_budget(problem_name: str):  # noqa: ANN201, D103
    astrodf_solver = instantiate_solver("ASTRODF")
    trust_region_solver = instantiate_solver("TrustRegion")

    problem = instantiate_problem(problem_name, problem_fixed_factors={"budget": 10000})
    [MRG32k3a() for _ in range(problem.model.n_rngs)]
...
def get_stopping_time(  # noqa: D417
    k: int,
    sig2: float,
    kappa: float,
    lambda_min: int,
    ...
    """Calculate the stopping time (sample size) based on current variance estimate,.
    ...
    pilot_run = math.ceil(
        max(
            lambda_min * math.log(10 + k, 10) ** 1.1, min(0.5 * problem.dim, lambda_max)
        )
        - 1
    )
...
def calculate_kappa(  # noqa: D417
    problem: Problem, sol: Solution, budget: int, k: int, lambda_min: int, delta: float
) -> None:
    """Calculate kappa and run adaptive sampling on the incumbent solution to obtain a.

    sample average of its response.
    ...
    while True:
        ...
        kappa = kappa.item()
        stopping = get_stopping_time(
            k, sig2, kappa, lambda_min, delta, problem, budget, used_budget
        )
...
def plot_adapt_sample_metric_multiple_problems(problem_names: list[str]) -> None:
    r"""Generates plots showing how the value of.

    $\frac{\lambda_k\hat{\sigma}^2(\mathbf{x},t)}{\kappa^2\Delta_k^2}$ changes as the
    monte carlo
    effort (t) increases at a design point $\mathbf{x}$ within an iteration k of the
    algorithm for multiple problems.
    """
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("adapt_sample_metric.png")
    plt.show()


def plot_adapt_sample_metric(problem_name: str):  # noqa: ANN201
    r"""Generates a plot showing how the value of.
    ...
    problem = instantiate_problem(problem_name, problem_fixed_factors={"budget": 10000})
    rng_list = [MRG32k3a() for _ in range(problem.model.n_rngs)]
    instantiate_solver(solver_name)

...
#! Change model construction and active subspace generation for the model-fitting method in ASTROMoRF 
...
class CallbackFunctor:
    def __init__(self, obj_fun, delta, U, x0):
        self.sols = []
...
def compress_points(sols, eps=0.1):
...
#    SUBPROBLEM PROCESS  #
##########################    
def objective_fn(problem: Problem, x:np.ndarray) -> float:
    #create solution object
    x = tuple(x.flatten())
    sol = Solution(x, problem)
    sol.attach_rngs(problem.rng_list)
    problem.simulate(sol,100)
    obj_fn = sol.objectives_mean.item()
    return obj_fn



def cand_soln_objective_fn(model, model_grad, x0, U, lmbda):
    if len(x0.shape) == 1:
        x0 = x0.reshape(-1, 1)

    n, d = U.shape  # n: full space dim, d: reduced space dim
    projected_x = (U.T @ x0).flatten() # Projected x0 in subspace
    original_x = x0.flatten()  # Original x0 in full space
    ...
    def obj_fn(x):
        ...
        print(f'Projected Step Size: {projected_step_size.flatten()}, \n Full Step Size: {full_step.flatten()}')
        print(f'projected step norm: {np.linalg.norm(projected_step_size, ord=2)}, full step norm: {np.linalg.norm(full_step, ord=2)}')
        print(f'Regularization Term: {np.linalg.norm(projected_step_size, ord=2)**2 / np.linalg.norm(full_step, ord=2)**2 }')

        reg = np.linalg.norm(projected_step_size, ord=2)**2 / np.linalg.norm(full_step, ord=2)**2    
...
def run_optimisation(model, model_grad, x0, U, delta, lmbda) :
    ...
    result = minimize(obj_fn, x_init.flatten(), method="trust-constr", jac=grad_fn,
...
#    RATIO COMPARISON   #
#########################
def ratio_comparison(problem: Problem, x0:np.ndarray, x_cand: np.ndarray, U:np.ndarray, model: callable) -> float:
    if len(x0.shape) == 1:
        x0 = x0.reshape(-1, 1)
...
#    PLOTTING FUNCTIONS   #
###########################
def plot_trajectory_solver(problem, model, x0, init_sol, U, delta, lmbda): 
    ...
    print("Final objective value at result:", objective_fn(problem, full_space_sol.flatten()))
    ...
    ax.grid(True)
    ax.set_title(rf"Trust-Region Subproblem Trajectory starting at $({x0[0]},{x0[1]:},{x0[2]})$ with $\lambda={lmbda}$")
    ax.set_xlabel("x1")
    ax.set_ylabel("x2")
    plt.tight_layout()
    plt.savefig("trust_region_trajectory.png")

def plot_lambda_vs_optimality_gap(problem, x0, U, delta, model, model_grad):
    ...
    [print(f'Lambda: {lmbda}, Ratio: {rat}') for lmbda, rat in zip(lambdas, ratios)]

    #plot a line of best fit through the (lambdas, opt_gaps) points
    sns.regplot(x=lambdas, y=ratios, scatter=False, ci=None, order=1, line_kws={'color':'red','linestyle':'--'})
...
def plot_trust_region(ax, delta, U, x0):
    if len(x0.shape) == 1:
        x0 = x0.reshape(-1, 1)
...
def plot_trajectory(ax, problem, cb):
    sols = np.array(cb.sols)
    # sols = compress_points(sols)

    # Plot internal trajectory points (bigger size = 80)
    if len(sols) > 2:
        ax.scatter(sols[1:-1, 0], sols[1:-1, 1], 
                   s=55, color='orange', label="Trajectory")
    ...
    ax.scatter(sols[0,0], sols[0,1], 
               color="green", marker='x', s=60, linewidths=2.5, 
               label="Initial Projected Solution")

    ax.scatter(sols[-1,0], sols[-1,1], 
               color="red", marker='x', s=60, linewidths=2.5, 
               label=f"Candidate Solution ({objective_fn(problem, sols[-1]):.2f})")

def plot_contours(ax, problem, x0, init_sol, U, delta):
    ...
    print(f'Plotting initial solution at: ({init_sol[0]}, {init_sol[1]}, {init_sol[2]})')
    ax.scatter(init_sol[0], init_sol[1], c="black", marker='*', s=100, label=f"Initial Solution ({objective_fn(problem, init_sol):.2f})")

    #plot the optimal solution 
    optimal_sol = problem.optimal_solution 
    print(f'Plotting optimal solution at: ({optimal_sol[0]}, {optimal_sol[1]}, {optimal_sol[2]})')
    ax.scatter(optimal_sol[0], optimal_sol[1], c="purple", marker='P', s=100, label=f"Optimal Solution ({objective_fn(problem, optimal_sol):.2f})")
    # ax.clabel(c, inline=True, fontsize=7, fmt="%.1f")


def main(): 
    x0 = np.array([-3.5, 3.5, 1.0, 0.5, -1.5, 2.0, 1.0, 0.5, -1.0, 0.0])  # Initial solution
    delta = 3.5
    lmbda = 0.0
    k = 1 
    ...
    problem = instantiate_problem(problem_name, problem_fixed_factors={'initial_solution': tuple(x0)})
    ...
    current_solution, delta_k, model, model_grad, U, fval, expended_budget, interpolation_solutions, visited_points_list  = construct_model(
		k, d, degree, problem, current_solution, delta, expended_budget, visited_points_list
	)

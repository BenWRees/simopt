...
def create_slurm_files(json_path: str, csv_file_name: str) -> list[str] : 
    ...
    with open(csv_file, newline='') as f:
        ...
        for i, row in enumerate(reader, start=1):
            ...
            # prepare arguments string in the form --problem_to_test <problem_name> --solver BO --proble_dim <dim_size> 
            # args = " ".join('"'+str(x)+'"' for x in row)
            solver_name = row[0]
            problem_name = row[1]
            dim_size = row[2]
            args = f' --solver {solver_name} --problem_to_test {problem_name} --problem_dim {dim_size} '

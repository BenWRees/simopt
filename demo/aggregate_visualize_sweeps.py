...
def load_json_list(path):  # noqa: ANN001, ANN201, D103
    if not path.exists():
        return []
...
def aggregate_coarse():  # noqa: ANN201, D103
    rows = []
...
def aggregate_hf():  # noqa: ANN201, D103
    rows = []
...
def write_csv(rows, out_path) -> None:  # noqa: ANN001, D103
    if not rows:
        return
    keys = list(rows[0].keys())
    with open(out_path, "w", newline="") as f:  # noqa: PTH123
...
def plot_by_dim(rows, out_dir, prefix) -> None:  # noqa: ANN001, D103

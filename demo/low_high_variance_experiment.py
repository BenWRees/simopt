...
-------
Tests the performance of ASTROMoRF against OMoRF for more deterministic problems vs more
stochastic problems
"""
...
def run_problem_solver_pair(  # noqa: D103
    solver_name: str, problem_name: str, variance: float
) -> None:
    print(f"Testing solver {solver_name} on problem {problem_name}.")
    if (
        solver_name == "ASTROMoRF" or solver_name == "OMoRF"
    ) and problem_name == "ROSENBROCK-1":
        solver_fixed_factors = {"subspace dimension": 7}

    elif (
        solver_name == "ASTROMoRF" or solver_name == "OMoRF"
    ) and problem_name == "DYNAMNEWS-1":
...
def main() -> None:  # noqa: D103
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for problem_name in problem_names:
            variances = [1.5, 17.5] if problem_name == "DYNAMNEWS" else [0.0, 5.0]

            for solver_name in solver_names:
                for variance in variances:
                    futures.append(
                        executor.submit(
                            run_problem_solver_pair, solver_name, problem_name, variance
                        )
                    )

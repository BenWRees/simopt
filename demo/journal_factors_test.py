"""
A script to test the performance of different initial subspace dimensions and different 
polynomial basis types for the ASTROMORF solver across a set of benchmark problems.
The script uses data farming to systematically vary the chosen factor and evaluate its impact
...
Usage:
	...
	python journal_factors_test.py --factor subspace --problem FACSIZE-1 --dim 10 --budget 5000
	
	# Test polynomial basis types
	python journal_factors_test.py --factor basis --problem FACSIZE-1 --dim 10 --budget 5000
	
	# For HPC array jobs (run a single design point)
	python journal_factors_test.py --factor subspace --problem FACSIZE-1 --dim 10 --budget 5000 --task-id 0
"""
...
class ExperimentConfig:
	...
	basis_types: list[PolyBasisType] = field(default_factory=lambda: list(PolyBasisType))
	...
	def __post_init__(self):
		...
		if self.factor_type not in ("subspace", "basis"):
			raise ValueError(f"factor_type must be 'subspace' or 'basis', got: {self.factor_type}")
	...
	try:
		...
		for pbt in PolyBasisType:
			if pbt.value.upper() == name_upper:
				return pbt
		raise ValueError(f"Unknown polynomial basis type: {name}")
...
def get_problem_fixed_factors(problem_name: str, dim: int, budget: int) -> dict[str, Any]:
...
def run_single_design_point(
	...
	experiment.log_experiment_results(file_path=output_file.with_suffix('.txt'))
...
def run_subspace_dimension_experiments(config: ExperimentConfig) -> list[dict[str, Any]]:
	"""
	...
	for dim in config.subspace_dims:
		...
		if dim <= config.problem_dim:
			solver_factors = {
				...
				"polynomial degree": problems_optimal_hyper.get(config.problem_name, {}).get('polynomial_degree', config.polynomial_degree),
	...
	if config.task_id is not None:
		if config.task_id < len(design_points):
			...
			path_file_name = f"{config.problem_name}_subspace_experiment_on_{design_point_id}"
			result = run_single_design_point(config, solver_factors, design_point_id, path_file_name)
			results.append(result)
		else:
			logger.warning(f"Task ID {config.task_id} out of range (max: {len(design_points) - 1})")
	else:
...
def run_polynomial_basis_experiments(config: ExperimentConfig) -> list[dict[str, Any]]:
	...
	logger.info(f"Testing basis types: {[POLY_BASIS_NAMES.get(b, str(b)) for b in config.basis_types]}")
	...
	if config.task_id is not None:
		if config.task_id < len(design_points):
			design_point_id, solver_factors = design_points[config.task_id]
			path_file_name = f"{config.problem_name}_polynomial_basis_experiment_on_{design_point_id}"
			result = run_single_design_point(config, solver_factors, design_point_id, path_file_name)
			results.append(result)
		else:
			logger.warning(f"Task ID {config.task_id} out of range (max: {len(design_points) - 1})")
...
def run_full_factorial_experiment(config: ExperimentConfig) -> list[dict[str, Any]]:
	...
	for dim in config.subspace_dims:
		...
		for basis_type in config.basis_types:
			...
			basis_name = POLY_BASIS_NAMES.get(basis_type, basis_type.value)
			design_point_id = f"ASTROMORF_d{dim}_basis_{basis_name}_on_{config.problem_name}"
			design_points.append((design_point_id, solver_factors))
	...
	if config.task_id is not None:
		if config.task_id < len(design_points):
			design_point_id, solver_factors = design_points[config.task_id]
			path_file_name = f"{config.problem_name}_full_factorial_experiment_on_{design_point_id}"
			result = run_single_design_point(config, solver_factors, design_point_id, path_file_name)
			results.append(result)
		else:
			logger.warning(f"Task ID {config.task_id} out of range (max: {len(design_points) - 1})")
...
def generate_hpc_config(config: ExperimentConfig, output_path: Path) -> Path:
	...
	if config.factor_type == "subspace":
		n_design_points = len([d for d in config.subspace_dims if d <= config.problem_dim])
	elif config.factor_type == "basis":
		n_design_points = len(config.basis_types)
	else:
		# Full factorial
		n_design_points = len([d for d in config.subspace_dims if d <= config.problem_dim]) * len(config.basis_types)
	...
	hpc_config = {
		...
		"subspace_dims": config.subspace_dims if config.factor_type in ("subspace", "full") else [config.fixed_subspace_dim],
		"basis_types": [b.value for b in config.basis_types] if config.factor_type in ("basis", "full") else [config.fixed_basis.value],
...
def generate_slurm_array_script(
	...
	if config.factor_type == "subspace":
		...
		n_tasks = len([d for d in config.subspace_dims if d <= config.problem_dim]) * len(config.basis_types)
...
def generate_csv_design_matrix(config: ExperimentConfig, output_path: Path) -> Path:
	...
	csv_file = output_path / f"design_matrix_{config.factor_type}_{config.problem_name}.csv"
...
Examples:
  ...
  python journal_factors_test.py --factor subspace --problem FACSIZE-1 --dim 30 --budget 10000 --generate-slurm
  
  # Run a specific task (for HPC array jobs)
  python journal_factors_test.py --factor subspace --problem FACSIZE-1 --dim 10 --task-id 3
		""",
	)
	...
	parser.add_argument(
		...
		required=True,
		help="Factor to test: 'subspace' for subspace dimensions, 'basis' for polynomial basis types, 'full' for factorial",
	)
	parser.add_argument(
		"--problem",
		type=str,
...
def main():
	...
	basis_types = None
	if args.basis_types:
		basis_types = [poly_basis_from_string(x.strip()) for x in args.basis_types.split(",")]
	else:
		basis_types = list(PolyBasisType)

...
def run_experiment(
    ...
    # These while loops are expected to constantly create new problem, model, and solver
    # factors until a solver factor works
    solver = None
    problem = None
    ...
    while problem is None:
        with contextlib.suppress(ValueError):
            problem = instantiate_problem(
                problem_name,
                update_problem_factor_dimensions(problem_name, dim_size, budget),
                update_model_factors_dimensions(problem_name, dim_size),
            )
    ...
    if solver_name == "ASTROMoRF" or solver_name == "OMoRF":
        solver.factors["initial subspace dimension"] = problems_optimal_hyper[
            problem_name
        ]["subspace_dimension"]
        solver.factors["polynomial degree"] = problems_optimal_hyper[problem_name][
            "polynomial_degree"
        ]
...
if __name__ == "__main__":
    ...
    main(
        solver_name, problem_name, dim_size, solver_factors, budget, macroreplication_no
    )

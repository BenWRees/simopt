...
def run_experiment(  # noqa: D417
    ...
    """Run an experiment of a solver on a problem and store the results in a .pickle.
    ...
    """
    file_name_path = f"{solver_name}_on_{problem_name}_crn_{'True' if solver_factors['crn_across_solns'] else 'False'}"  # noqa: E501
    ...
    if solver_name == "ASTROMORF":
        ...
        if "subspace_dimension" in optimal_hyper:
            solver_factors["initial subspace dimension"] = optimal_hyper[
                "subspace_dimension"
            ]
    ...
    try:
        ...
        raise KeyError(
            f"Unknown solver code '{solver_name}'. Ensure it matches a class_name_abbr and is present in solver_renames."  # noqa: E501
        ) from e
    except ValueError as e:
        raise RuntimeError(f"Failed to instantiate solver '{solver_name}': {e}") from e

    problem = scale_dimension(
        problem_name=problem_name, dimension=problem_dim, budget=budget
    )

    print(
        f"Running {solver_name} on {problem_name} with budget {budget}, dim_size {solver.factors.get('initial subspace dimension', 'N/A')}, polynomial basis {solver_factors.get('polynomial basis', 'N/A')} for {macroreplication_no} macroreplications."  # noqa: E501
    )
    ...
    if getattr(myexperiment, "n_macroreps", 0) >= macroreplication_no:
        print(
            f"Running Post-Replications and post-normalization and saving to {checkpoint_path}"  # noqa: E501
        )
        postrep_no = macroreplication_no * 20
        myexperiment.post_replicate(n_postreps=postrep_no)
        file_name_pickle = f"{solver_name}_ON_{problem_name}_CRN({solver_factors['crn_across_solns']})_POSTREPS.pickle"  # noqa: E501
        if output_dir:
            file_name_pickle = str(Path(output_dir) / file_name_pickle)
        myexperiment.record_experiment_results(file_name_pickle)
        myexperiment.log_experiment_results(
            file_path=file_name_path.replace(".pickle", ".txt")
        )
    else:
        print(
            f"Not enough macroreplications done to run post-replications. Only {myexperiment.n_macroreps} done out of {macroreplication_no} requested."  # noqa: E501
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run OMoRF pickle generation for a solver/problem"
    )
    parser.add_argument("solver_name", type=str)
    parser.add_argument("problem_name", type=str)
    parser.add_argument("dim_size", type=str, help="Dimension size or 'None'")
    parser.add_argument(
        "solver_factors", type=str, help="Python dict literal of solver fixed factors"
    )

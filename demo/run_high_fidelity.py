...
# Ensure the demo folder is on sys.path so we can import the tuning helper
sys.path.append(str(Path(__file__).resolve().parent))
from tune_and_eval_adaptive_d import run_tuning
...
for f in coarse_files:
    ...
    with open(f) as fh:
        data = json.load(fh)
    dim = int(os.path.splitext(os.path.basename(f))[0].split('dim')[-1])
    for entry in data:
        entry['dim'] = dim
...
for dim, combos in selected.items():
    ...
    for (budget, cp, power, alpha) in combos:
        ...
        res = run_tuning(n_trials=200, dim=dim, budgets=[budget], cps=[cp], powers=[power], alphas=[alpha])
        hf_results.extend(res)
    out = os.path.join(out_dir, f'hf_results_dim{dim}.json')
    with open(out, 'w') as fh:
        json.dump(hf_results, fh)
    summary[dim] = {'file': out, 'runs': len(hf_results)}

summary_file = os.path.join(out_dir, 'hf_summary.json')

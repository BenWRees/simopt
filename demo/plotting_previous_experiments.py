...
-------
This script offers the chance to plot the experiments after running (as I expect I will
make a mistake in the demo_problems_solver.py file)
...
def get_pickle_files(folder_path):  # noqa: ANN001, ANN201, D103
    return [
        os.path.join(folder_path, file)  # noqa: PTH118
        for file in os.listdir(folder_path)  # noqa: PTH208
...
def sort_problem_solver(  # noqa: D103
    problem_solvers: list[ProblemSolver],
) -> list[list[ProblemsSolvers]]:
...
def post_replications(experiments: list[ProblemSolver]) -> list[ProblemSolver]:  # noqa: D103
...
def plot_group(problemSolvers: ProblemsSolvers) -> list[str]:  # noqa: D103, N803
    ...
    ouput_path_1 = plot_solvability_profiles(
        ...
        plot_title="Solvability Profiles of Problem Solvers",
    )
    ouput_path_2 = plot_solvability_profiles(
        ...
        plot_conf_ints=False,
        legend_loc="upper left",
        plot_title="Quantile Solvability Profiles of Problem Solvers",
    )
    return [ouput_path_1, ouput_path_2]


def plot_individual(experiments: list[ProblemSolver]) -> list[str]:  # noqa: D103
    ...
    for exp in experiments:
        ...
        plot_progress_curves(
            ...
            normalize=False,
            plot_title=f"All {solver_name} on {problem_name}",
        )
        ...
        mean_output_name = plot_progress_curves(
            ...
            normalize=True,
            plot_title=f"{solver_name} on {problem_name}",
        )
...
def main() -> None:  # noqa: D103
    ...
    [
        print(f"problem name is {a.problem.name} \t and solver name is {a.solver.name}")
        for a in myexperiment_list
    ]
    ...
    # print(f'The optimal solution is {ps.optimal_solution} and the optimal value is
    # {ps.optimal_value}')
    ...
    # myexperiments = ProblemsSolvers(experiments=sort_problem_solver(problem_solvers),
    # file_name_path='ProblemSolvers')

    # for exp_group in myexperiments.experiments :
    # 	for exp in exp_group :
    # print(f'The solver name is {exp.solver.name} and the problem name is
    # {exp.problem.name}')

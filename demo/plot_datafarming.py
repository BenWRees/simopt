...
def _as_experiment_list(
    experiments: list[ProblemSolver] | ProblemsSolvers,
) -> list[ProblemSolver]:
    """Normalize either ProblemsSolvers or flat list input to a flat list."""
    if hasattr(experiments, "experiments"):
        nested = cast(list[list[ProblemSolver]], experiments.experiments)
        flat: list[ProblemSolver] = []
        for solver_exps in nested:
            flat.extend(solver_exps)
        return flat
    return experiments
...
def extract_optimality_gap_fraction(
    ...
    # Avoid using truth-value checks on numpy arrays (they raise ambiguous truth value
    # errors).
    x0_postreps = getattr(experiment, "x0_postreps", None)
    xstar_postreps = getattr(experiment, "xstar_postreps", None)

    x0_obj = (
        float(np.mean(x0_postreps))
        if (x0_postreps is not None and len(x0_postreps) > 0)
        else None
    )
    xstar_obj = (
        float(np.mean(xstar_postreps))
        if (xstar_postreps is not None and len(xstar_postreps) > 0)
        else None
    )
    ...
    for mrep in range(experiment.n_macroreps):
        if len(experiment.all_est_objectives[mrep]) > 0:
            ...
            if abs(total_gap) < 1e-10:
                ...
                # Clamp to [0, 1.5] range (can be negative if terminal is better than
                # x*)
...
def extract_fraction_closed(
    ...
    """Extract the fraction of the optimality gap that has been closed for each.

    macroreplication.

    Fraction closed is defined for minimization as:
        (f(x0) - f(x_t)) / max(f(x0) - f(x*), eps)
    where x0 is the initial solution, x_t is the terminal solution, and x* is the best
    known solution.

    Returns values in [0, 1] (clamped). 1.0 indicates the initial gap has been fully
    closed.
...
def extract_convergence_rate(
    experiment: ProblemSolver,
    method: Literal[
        "linear_fit", "exponential_fit", "area_ratio", "budget_to_threshold"
    ] = "area_ratio",
    ...
    for mrep in range(experiment.n_macroreps):
        ...
        if method == "linear_fit":
            ...
            budget_fractions = budgets / total_budget
            area = np.trapezoid(progress, budget_fractions)
            ...
            # Find the fraction of budget needed to reach threshold_fraction of
            # improvement
            progress = (initial_obj - est_objectives) / total_improvement
            budget_fractions = budgets / total_budget
...
def plot_datafarming_results(  # noqa: D417
    experiments: list[ProblemSolver] | ProblemsSolvers,
    factor_name: str,
    y_metric: Literal[
        "terminal_objective",
        "optimality_gap_fraction",
        "fraction_closed",
        "convergence_rate",
    ] = "terminal_objective",
    convergence_method: Literal[
        "linear_fit", "exponential_fit", "area_ratio", "budget_to_threshold"
    ] = "area_ratio",
    title: str | None = None,
    ...
    """Plot data farming results with factor values on x-axis and performance metric on.
    ...
    Args:
        ...
        y_metric: What to plot on the y-axis:
            ...
            - "optimality_gap_fraction": Remaining optimality gap (0 = optimal, 1 = no
            improvement)
            - "convergence_rate": Rate of convergence (method depends on
            convergence_method)
        convergence_method: Method to compute convergence rate (only used if
        y_metric="convergence_rate"):
            ...
            - "budget_to_threshold": Fraction of budget to reach 90% improvement (lower
            = faster)
    ...
    Example:
        ...
        >>> experiments = [read_experiment_results(f"exp_{i}.pickle") for i in range(1,
        9)]
        ...
        ... )
    """
    experiment_list = _as_experiment_list(experiments)
    ...
    for exp in experiment_list:
        ...
        if y_metric == "terminal_objective":
            # Aggregate terminal objective per Problem-Solver pair (one point per
            # experiment)
    ...
    if groups_by_problem:
        ...
        colors = plt.get_cmap("tab10")(np.linspace(0, 1, len(groups_by_problem)))

        if is_numeric:
            # Numeric factor: plot per-problem using only that problem's factor levels
            for (problem_name, grp), color in zip(
                groups_by_problem.items(), colors, strict=False
            ):
                # Attempt to interpret group keys as numeric factors
                keys = list(grp.keys())
                # Try converting keys to floats; fall back to original order if
                # conversion fails
                try:
                    numeric_keys = sorted([float(k) for k in keys])
                    {float(k): k for k in keys}
                except Exception:
                ...
                if show_confidence_interval and ns:
                    ci_multiplier = stats.t.ppf(
                        (1 + confidence_level) / 2, df=np.maximum(np.array(ns) - 1, 1)
                    )
                    ci_halfwidth = (
                        ci_multiplier
                        * np.array(stds)
                        / np.sqrt(np.maximum(np.array(ns), 1))
                    )
                    ax.fill_between(
                        x_mean_vals,
            ...
            unique_factors = sorted(all_factors, key=str)
            ...
            ax.set_xticklabels(
                [str(fv) for fv in unique_factors],
                rotation=45,
                ha="right",
            )
        ...
        if y_scale == "log":
            ...
            if min_y is None or min_y > 0:
                ...
                print(
                    "Warning: data contains non-positive values; using 'symlog' fallback for y-axis scaling."  # noqa: E501
                )
        ...
        ax.set_ylabel(ylabel if ylabel is not None else "Value", fontsize=12)
    ...
    if is_numeric:
        ...
        if show_confidence_interval:
            ...
            ci_multiplier = stats.t.ppf(
                (1 + confidence_level) / 2, df=np.maximum(ns - 1, 1)
            )
        ...
        ax.set_xticks(x_positions)
        ax.set_xticklabels([str(fv) for fv in unique_factors], rotation=45, ha="right")
...
def plot_datafarming_comparison(
    ...
    y_metric: Literal[
        "terminal_objective", "optimality_gap_fraction", "fraction_closed"
    ] = "terminal_objective",
    title: str | None = None,
    ...
    for (group_name, group_exps), color in zip(groups.items(), colors, strict=False):
        ...
        stds = [
            np.std(factor_to_y[f], ddof=1) if len(factor_to_y[f]) > 1 else 0
...
def create_results_dataframe(
    ...
    for exp in experiment_list:
        ...
        try:
            conv_rates_budget = extract_convergence_rate(
                exp, method="budget_to_threshold"
            )
        except ValueError:
...
def load_experiments_from_directory(
    ...
    pickle_files: list[str | Path] = list(directory.glob(pattern))

...
def eval_with_power(n_trials=200, dim=10, cp=0.1, power=2.0):  # noqa: ANN001, ANN201, D103
    random.seed(1)
    np.random.seed(1)
    exact = 0
    within1 = 0
    ...
    for _t in range(n_trials):
        ...
        for d in range(1, dim):
            ...
            if max_val - min_val < 1e-12:
                ...
                s_val = 1.0 - (val_err - min_val) / max(1e-12, (max_val - min_val))
            s_proj = 1.0 - proj_resid
            s_succ = 0.0
    ...
    for dim in dims:
        for budget in budgets:
            for cp, power in tests:
                # adapt eval to pass budget into DummyProblem used inside
                # eval_with_power
                acc, w1, mr, h = eval_with_power(200, dim, cp=cp, power=power)
                # note: eval_with_power uses e.make_spectrum and make_validation; solver
                # in eval uses DummyProblem without budget here
                # we will print requested parameters for clarity
                print(
                    f"dim={dim} budget={budget} cp={cp} power={power}: exact={acc:.3f}, within1={w1:.3f}, mean_regret={mr:.5f}, top={sorted(h.items(), key=lambda kv: -kv[1])[:5]}"  # noqa: E501
                )

...
def run_different_subspaces(  # noqa: D103
    solver_name: str, problem: Problem, subspace_dim: int
) -> ProblemSolver:
    ...
    new_solver_name = (
        solver_name + " on " + problem.name + " with subspace " + str(subspace_dim)
    )
    file_name = new_solver_name.replace(" ", "_") + ".pickle"
    return ProblemSolver(
        solver_name=solver_name,
        solver_rename=new_solver_name,
        file_name_path=file_name,
        problem=problem,
        solver_fixed_factors={"subspace dimension": subspace_dim},
    )


def run_problem_solver(myexperiment: ProblemSolver) -> None:  # noqa: D103
...
def main() -> None:  # noqa: D103
    ...
    for problem_name in [
        problem_names
    ]:  # ['DYNAMNEWS-1', 'ZAKHAROV-1', 'ROSENBROCK-1'] :
        if problem_name == "DYNAMNEWS-1":
            problem = problem_directory[problem_name](
                fixed_factors={"budget": 600}, model_fixed_factors={"mu": 18.0}
            )
        elif problem_name == "ZAKHAROV-1" or problem_name == "ROSENBROCK-1":
            problem = problem_directory[problem_name](
                fixed_factors={"budget": 600}, model_fixed_factors={"variance": 5.0}
            )
        else:
        ...
        for solver_name in [solver_names]:  # ['ASTROMoRF', 'OMoRF'] :
            for dim in range(1, problem.dim + 1):
                problem_solvers_list.append(
                    run_different_subspaces(solver_name, problem, dim)
                )

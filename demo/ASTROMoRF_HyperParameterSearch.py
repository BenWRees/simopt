...
and polynomial degree for ASTROMoRF on the specified problem using Bayesian
optimization.
After running the hyperparameter search it prints the final solution found (this is the
best dimension and degree).
"""
...
def main(problem_to_test: str, solver: str, problem_dim: int | None = None) -> None:
    """Run hyperparameter optimization for ASTROMoRF on a selected problem."""
    print("Running Hyperparameter Test.")
    macroreplication_no = 1

    # Specify file path name for storing experiment outputs in .pickle file.
    file_name_path = "experiments/outputs/Hyperparameter_Test.pickle"
    print(f"Results will be stored as {file_name_path}.")

    if problem_dim is not None:
        target_problem_model_factors = update_model_factors_dimensions(
            problem_to_test, problem_dim
        )
        target_problem_factors = update_problem_factor_dimensions(
            problem_to_test, problem_dim, 4000
        )
    else:
    ...
    print(
        f"The model factors of {problem_to_test} are set to: \n {target_problem_model_factors} \n"  # noqa: E501
    )
    print(
        f"The problem factors of {problem_to_test} are set to: \n {target_problem_factors} \n"  # noqa: E501
    )

    target_problem = instantiate_problem(
        problem_name=problem_to_test,
        problem_fixed_factors=target_problem_factors,
        model_fixed_factors=target_problem_model_factors,
    )
    ...
    if solver == "RANDS":
        ...
        model_fixed_factors = {  # For random search we only want to look at the degree and dimension  # noqa: E501
        ...
        print(
            f"Starting Hyperparameter Test on problem {problem_to_test} using solver {solver}..."  # noqa: E501
        )
        myexperiment.run(n_macroreps=macroreplication_no)

        print(
            f"Completed {macroreplication_no} macroreplications. Now running post-replications to finalize results..."  # noqa: E501
        )

        myexperiment.post_replicate(n_postreps=50)

        terminal_vals = [
            myexperiment.all_recommended_xs[i][-1]
            for i in range(len(myexperiment.all_recommended_xs))
        ]
        # out of the terminal_vals select the integer (dimension, degree) pair that is
        # recommended most often
        final_solution = max(set(terminal_vals), key=terminal_vals.count)
        print(
            f"Finished running Hyperparameter Test. Final solution found: {final_solution}"  # noqa: E501
        )
        ...
        # Initialize an instance of the experiment class. - optimising over gamma values
        # too
        myexperiment = ProblemSolver(
            solver,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Compute optimal active subspace dimension and polynomial degree for a given problem."  # noqa: E501
    )
    parser.add_argument(
        "--problem",
        type=str,
    ...
    parser.add_argument(
        "--problem_dim", type=int, help="dimension of Problem", default=None
    )

...
class TrustRegionBase(Solver) :
	...
	def evaluate_candidate_solution(self, model, problem, fval_tilde, delta_k, interpolation_solns, current_solution, candidate_solution) :
		"""
...
class TrustRegion(TrustRegionBase) :
	...
	def solve(self, problem: Problem) -> tuple[list[Solution], list[int]] :
		...
		while self.budget.remaining > 0 :
			...
			if k == 1 and hasattr(sampling_instance.__class__, 'calculate_kappa'):
				...
				current_solution = sampling_instance.calculate_kappa(k, problem, current_solution, delta_k)
				...
				while True:
					...
					stopping = sampling_instance.get_stopping_time(sig2, delta_k, k, problem)
			...
			#build random model 
			current_solution, delta_k, interpolation_solns, visited_pts_list = model.construct_model(current_solution, delta_k, k, visited_pts_list)
			...
			candidate_solution, fval_tilde = self.sample_candidate_solution(sampling_instance, candidate_solution, current_solution, problem, k, delta_k)
			...
			current_solution, delta_k = self.evaluate_candidate_solution(model, problem, fval_tilde, delta_k, interpolation_solns, current_solution,\
																			   candidate_solution)	
	...
	def sample_candidate_solution(self, sampling_instance, candidate_solution, current_solution, problem, k, delta_k) : 
			if self.factors['crn_across_solns'] :
				...
				candidate_solution, expended_budget = sampling_instance(problem, k, candidate_solution, delta_k)
				fval_tilde = -1 * problem.minmax[0] * candidate_solution.objectives_mean
				return candidate_solution, fval_tilde

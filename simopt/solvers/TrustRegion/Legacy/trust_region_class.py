...
class trust_region(Solver) :
	...
	def specifications(self) -> dict[str, dict] :
		return {
			...
			"poly_basis": {
				...
				"datatype": str, 
				"default": "LagrangePolynomialBasis"
			}, 
			"random_model type" : {
				"description": "The type of random model used",
				"datatype": str,
				"default": "simopt.solvers.GP_Model:GP_Model" 
			},
	...
	def solve_subproblem(self, delta, model, problem, solution, visited_pts_list) :
		...
		if self.factors['random_model type'].split(':')[1] == 'GP_model' : 
			def subproblem(s) : 
		...
		if self.factors['random_model type'].split(':')[1] == 'random_model_reuse' :

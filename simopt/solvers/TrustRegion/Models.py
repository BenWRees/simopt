...
class RandomModel:
    ...
    def construct_model(  # noqa: D102
        ...
        while True:
            ...
            for i in range(len(Y)):
                ...
                if (k == 1) and (i == 0):
                    ...
                    fval.append(
                        -1 * self.problem.minmax[0] * current_solution.objectives_mean
                    )
                    interpolation_solns.append(current_solution)

                # Otherwise, we need to simulate the system
                else:
                    interpolation_pt_solution = self.tr_instance.create_new_solution(
                        tuple(Y[i]), self.problem
                    )
                    # check if there is existing result
                    # self.problem.simulate(interpolation_pt_solution, 1)

                    interpolation_pt_solution = self.sampling_instance(
                        self.problem, k, interpolation_pt_solution, delta_k
                    )

                    fval.append(
                        -1
                        * self.problem.minmax[0]
                        * interpolation_pt_solution.objectives_mean
                    )
            ...
            if not self.model_construction_parameters["skip_criticality"]:  # noqa: SIM102
                ...
                if (
                    norm(grad)
                    > self.model_construction_parameters["criticality_threshold"]
                ):
                    break
        ...
        self.fval = fval
        delta_k = min(
            max(self.model_construction_parameters["beta"] * norm(grad), delta_k), delta
        )
...
# 		# self.sampling_instance.calculate_pilot_run(k, self.problem, expended_budget)
...
# 					current_solution, expended_budget = self.sampling_instance(self.problem, k, current_solution, delta_k, expended_budget, False)
...
# 					#SAMPLING STRAT 2
# 					reuse_solution, expended_budget = self.sampling_instance(self.problem, k, reuse_solution, delta_k, expended_budget, False)
...
# 					interpolation_pt_solution, expended_budget = self.sampling_instance(self.problem, k, interpolation_pt_solution, delta_k, expended_budget)

...
class RandomModel :
	...
	def construct_model(self, current_solution, delta, k, visited_pts_list) -> tuple[
		Solution,
		float,
		int,
		list[Solution],
		...
		while True:
			...
			for i in range(len(Y)):
				...
				# Otherwise, we need to simulate the system
					...
					interpolation_pt_solution = self.sampling_instance(self.problem, k, interpolation_pt_solution, delta_k)
	...
	def coefficient(self, Y: list[np.ndarray], fval: list[float]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
		...
		M = self.poly_basis.V(Y, d) # now constructs M based on the polynomial basis being used

...
UPDATES
-------
- Added adaptive subspace dimension adjustment based on solver progress and model quality.
- Implemented plateau detection mechanism to reset subspace dimension when solver progress stalls.
- Enhanced tracking of model quality metrics to inform dimension adjustment decisions.
    **The adaptive dimension logic monitors the solver's progress and adjusts the subspace
    ...
    focus on a more promising subspace. This adaptive strategy aims to balance exploration
...
from scipy.special import factorial, betainc, gamma
...
class ASTROMoRFConfig(SolverConfig):
    ...
    gamma_3: Annotated[
        ...
        Field(
            ...
            gt=0,
            lt=1,
            description="trust-region radius decrease rate after unsuccessful iteration",
        ),
    ]
    ...
    variance_explained_threshold: Annotated[
        ...
        Field(
            ...
            ge=0.5,
            le=1.0,
            description="fraction of variance to capture when determining optimal subspace dimension",
...
class ASTROMORF(Solver):
    ...
    @property
    def prev_U(self) -> np.ndarray | None:
        """Get the previous active subspace matrix."""
        return self._prev_U

    @prev_U.setter
    def prev_U(self, value: np.ndarray | None) -> None:
        """Set the previous active subspace matrix."""
        self._prev_U = value

    @property
    def prev_H(self) -> np.ndarray | None:
        """Get the previous model Hessian."""
        return self._prev_H

    @prev_H.setter
    def prev_H(self, value: np.ndarray | None) -> None:
    ...
    def _set_basis(self, basis: PolyBasisType, problem: Problem | None = None) -> None:
    ...
    def _initialize_solving(self) -> None:
        ...
        self.previous_model_information: list = []  # Store previous model info for dimension decisions
        
        # Enhanced tracking for adaptive dimension stopping rule
        # Maps dimension d -> list of (iteration, residual_norm, prediction_error, success)
        self.dimension_performance: dict[int, list[tuple[int, float, float, bool]]] = {}
        # Store eigenvalue spectrum from gradient outer products for variance-explained stopping rule
        ...
        # Threshold for variance explained stopping rule (fraction of total variance to capture)
        self.variance_explained_threshold: float = self.factors.get("variance explained threshold", 0.95)
        ...
        self.initial_subspace_dimension: int = self.d  # Store initial d for reset target
        self.recent_objective_values: list[float] = []  # Track recent objective values for plateau detection
        ...
        self.plateau_threshold: float = 0.01  # Relative improvement threshold (0.5% - less strict than 0.1%)
        self.in_dimension_reset: bool = False  # Whether we're in a dimension reset cycle
        ...
        self.max_dimension_resets: int = 5  # Maximum number of resets allowed per run (increased from 3)
    ...
    def solve(self, problem: Problem) -> None:
        ...
        try:
            while self.budget.remaining > 0:
                ...
                # Build random model
                U, fval, interpolation_solns, X, fX = (
                ...
                if self.factors["Record Diagnostics"]:
                    # diagnostics.diagnose_model_quality expects (model, model_grad, X, fX, U)
                    self.diagnostics.diagnose_model_quality(self.model, self.model_grad, X, fX, U)
                ...
                #             (np.array([self.model(x.reshape(-1, 1)) for x in X]) - fX.flatten()) ** 2
                #         ))
                #     except Exception:
                #         residual_norm = 0.0
                ...
                #     grad_norm = norm(self.model_grad(np.zeros((1, self.problem.dim)), full_space=True))
                ...
                """
                    If the trust-region is still large and the subspace dimension is high,
                    reduce the polynomial degree to 2 to improve model accuracy and stability.
                    This should follow the STRONG
                """
                if self.delta >= self.delta_initial / 2 and self.d > self.problem.dim * 0.6 and self.degree != 2:
                    self.degree = 2
                else : 
    ...
    def record_iteration_performance(self, trust_region: float, current_solution: Solution, success: bool) -> None: 
        """
            Record the performance of the current iteration to be used in deciding optimal subspace dimensions
            Record in the self.previous_model_information structure a dictionary of the following format: 
            {
                ...
                'solution': a Solution object of the incumbent solution at the end of the iteration,
                'model_success': a bool indicating whether the iteration was successful,
                'eigenvalue_spectrum': a list[float] of the eigenvalues of the gradient outer product matrix of the model,
                'recommended_dimension': an int of the posterior recommended subspace dimension, 

            }
        """
        current_iteration_info = dict() 
        ...
        recommended_dimension = np.searchsorted(variance_ratios, self.variance_explained_threshold) + 1
        ...
        if hasattr(self, 'last_validation_by_d') and isinstance(self.last_validation_by_d, dict):
    ...
    def compute_eigenvalue_spectrum_model(self, current_solution: Solution, delta: float) -> list[float]:
        """
        ...
        no_pts = max(self.problem.dim, int(6 * np.log(max(self.problem.dim, 2)) * self.problem.dim))
        ...
        X = np.array([x_k.flatten() + s for s in shifts])
        ...
        for x in X:
            try:
                ...
                g = np.zeros(self.problem.dim)
            grads.append(g)
        G = np.vstack(grads)  # shape (m, n)
        C = (G.T @ G) / G.shape[0] # shape (n, n)
    ...
    def check_intersecting_trust_regions(self, centre_to_check: Solution, radius_to_check: float) -> bool : 
        """
            Check if the trust-region defined by centre_to_check and radius_to_check intersects with the current trust-region and solution
        Args:
            centre_to_check (Solution): A previous iteration's solution (centre of trust-region)
    ...
    def intersection_amount(self, model_info: dict) -> float:
        """
            Compute the amount of intersection between the trust-region defined by model_info and the current trust-region
        Args:
        ...
        if d <= abs(model_delta - self.delta):
            return 1.0 if self.delta <= model_delta else (model_delta / self.delta) ** self.problem.dim

        # Cap heights
        h_r = (model_delta + self.delta - d) * (self.delta - model_delta + d) / (2 * d)
        h_R = (model_delta + self.delta - d) * (model_delta - self.delta + d) / (2 * d)

        # Regularized beta arguments
        z_r = 1.0 - (1.0 - h_r / self.delta) ** 2
        z_R = 1.0 - (1.0 - h_R / model_delta) ** 2
        ...
        cap_R = 0.5 * (model_delta / self.delta) ** self.problem.dim * betainc(a, b, z_R)

        return cap_r + cap_R
    
    def obtain_intersecting_models(self, successful_models: list[dict]) -> tuple[list[dict],list[float]]:
        """
            Obtain the list of previous models whose trust-regions intersect with the current trust-region
            and order the list by how much they intersect the current trust-region by descending order 
        Args:
            successful_models (list[dict]): A list of previous model information dictionaries that were successful
        Returns:
            tuple[list[dict], list[float]]: A tuple containing a list of previous model information dictionaries that intersect 
            with the current trust-region and a list of their corresponding intersection amounts
        """
        #Populate list of intersecting models
        intersecting_models = []
        for info in successful_models:
            if self.check_intersecting_trust_regions(info['solution'], info['trust_region_radius']):
                intersecting_models.append(info)

        #Order intersecting models by how much they intersect the current trust-region by descending order
        intersecting_models.sort(key=self.intersection_amount, reverse=True)

        #get the intersection amounts 
        intersection_amounts = [self.intersection_amount(info) for info in intersecting_models]

        return intersecting_models, intersection_amounts

    #! THIS LOGIC NEEDS TO BE FILLED IN
    def infer_optimal_dimension_from_models(self, intersecting_models: list[dict]) -> int:
        """
            Infer the optimal subspace dimension based on intersecting models' recommended dimensions
        Args:
            intersecting_models (list[dict]): A list of previous model information dictionaries that intersect with the current trust-region

        Returns:
            int: The inferred optimal subspace dimension
        """
        recent_eigenvalues = [eigenvals for model_info in intersecting_models for eigenvals in [model_info['eigenvalue_spectrum']]] 
        ...
        optimal_d = np.searchsorted(cumulative_variance, self.variance_explained_threshold) + 1
        optimal_d = min(optimal_d, self.problem.dim - 1) #ensure not exceeding problem.dim -1
        return optimal_d
    ...
    def evaluate_and_score_candidate_dimensions(self) -> dict:
        """
            ...
            Returns:
                dict: mapping d -> {"val_err":..., "proj_resid":..., "success_rate":..., "score":...}
        """
        results = {}
        ...
        if eig_source is None:
            ...
            spectra = [np.array(info.get("eigenvalue_spectrum", []), dtype=float) for info in self.previous_model_information if info.get("eigenvalue_spectrum")]
            if spectra:
                # pad to same length
                maxlen = max(s.shape[0] for s in spectra)
                padded = np.vstack([np.pad(s, (0, maxlen - s.shape[0]), constant_values=0.0) for s in spectra])
        ...
        sufficient = [d for d, m in results.items() if float(m.get("score", 0.0)) >= threshold]
    ...
    def compute_optimal_subspace_dimension(self) -> None:
        """
            Compute the optimal subspace dimension for the current iteration based on previous model information.
        Returns:
            int: The optimal subspace dimension for the current iteration.
        TODO: Complete the fallback logic for cases with no successful or intersecting models
        TODO: Add in logic to handle plateau detection and dimension resets
        """
        # Use scoring across candidate dimensions combining validation, projection residual and success history
        # Plateau and reset logic preserved
        # Relaxed safety: shorter cooldown between d-changes to allow faster adaptation
        min_iters_between_d_changes = 1
        ...
        if self.in_dimension_reset:
            ...
            if not self.previous_model_information and not hasattr(self, "last_validation_by_d"):
                return self.d
            ...
            # Write diagnostic summary of candidate scores to diagnostics file (if enabled)
            try:
                if self.factors.get("Record Diagnostics", False) and hasattr(self, "diagnostics"):
        ...
        # If optimal_d equals current dimension, optionally allow small forced exploration
        if optimal_d == self.d:
            if (
                (self.iteration_count - getattr(self, "last_d_change_iteration", 0)) > 5
        ...
        if optimal_d != self.d:
            print(f'Iteration {self.iteration_count}: Adaptive subspace dimension change: {self.d} -> {optimal_d}')
            logging.info(
                f"Adaptive subspace: {'Increasing' if optimal_d > self.d else 'Decreasing'} "
    ...
    def heuristic_for_no_successful_models(self) -> int:
        """
            Heuristic to determine optimal subspace dimension when no successful models exist.
        ...
        if hasattr(self, "ema_pred_error"):
            self.ema_pred_error = ema_alpha * (np.abs(self.recent_prediction_errors[-1]) if self.recent_prediction_errors else 0.0) + (1 - ema_alpha) * self.ema_pred_error
        else:
            self.ema_pred_error = np.mean(self.recent_prediction_errors[-3:]) if len(self.recent_prediction_errors) >= 1 else 0.0
        ...
        # Bound and return
        target_d = max(1, min(self.problem.dim - 1, int(target_d)))
        return target_d

    def heuristic_performance_with_successful_models(self, successful_models: list[dict]) -> int:
        """
            Heuristic to determine optimal subspace dimension based on successful models' performance.
            This considers recent prediction errors and trust-region sizes.

            We consider: 
            1. The smallest dimension that has been tried multiple times and led to successful results
            2. The dimension that led to the lowest average prediction error among successful models
            3. The dimension that aligns with the current trust-region size (smaller dimensions for smaller trust-regions)
        ...
        #1. Find the smallest dimension if it has been tried multiple times and lead to successful results 
        #filter d_history for successful iterations
        successful_d_history = [self.d_history[i-1] for i in self.successful_iterations if i-1 < len(self.d_history)]
        d_counts = Counter(successful_d_history)
        smallest_successful_d = min((d for d, count in d_counts.items() if count >= 2), default=self.d)
        candidate_dimensions.append(smallest_successful_d)


        #2. The dimension that led to the lowest average prediction error among successful models
        # Use per-model stored recommended_dimension when available; use global recent errors as fallback
        d_pred_errors = {}
        for info in successful_models:
            d = info['recommended_dimension']
            #get the prediction error from the recent_prediction_errors list corresponding to this model
            model_pred_error = np.mean(self.recent_prediction_errors[-5:]) if self.recent_prediction_errors else 0.0
        ...
        if len(self.successful_iterations) >= 3:
            recent_successful_ds = [self.d_history[i-1] for i in self.successful_iterations[-3:] if i-1 < len(self.d_history)]
        ...
        optimal_d = max(1, min(self.problem.dim - 1, int(optimal_d)))
        return optimal_d



    def adaptive_dimension_successful_intersecting_models(self, intersecting_models: list[dict], intersection_amounts: list[float]) -> int:
        """
            Determine the optimal subspace dimension based on successful intersecting models. 
            We consider three strategies:
            1. Infer optimal dimension from the intersecting models' average eigenvalue spectra
            2. Check for consensus among intersecting models' recommended dimensions which significantly intersect the current trust-region
            3. Look for patterns in dimension changes among intersecting models

            We see out of these three candidates which one is most agreed upon, and if all are different, we use a scoring system to select the best candidate.
            The scoring system rewards:
            - Higher dimensions when trust-region is small and smaller dimensions when trust-region is large
            - Dimensions suggested by models with higher intersection amounts

        Args:
            intersecting_models (list[dict]): A list of previous model information dictionaries that were successful
            intersection_amounts (list[float]): A list of intersection amounts with the current trust-region corresponding to the successful models
        ...
        #obtain list of optimal dimensions from intersecting models sorted by how much they intersect the current trust-region
        intersecting_optimal_ds = [info.get('recommended_dimension', self.d) for info in intersecting_models]

        # see if there is some consensus among intersecting models
        # Consider only models with significant intersection (e.g., intersection amount >= 0.5)
        significant_ds = [d for d, amt in zip(intersecting_optimal_ds, intersection_amounts) if amt >= 0.5]
        if significant_ds:
            dim_counts = Counter(significant_ds)
            optimal_d_2, count = dim_counts.most_common(1)[0]
            candidates.append(optimal_d_2)

        #see if there is any pattern that can be extrapolated from the change in dimensions of intersecting models from smallest intersection
        #amount to largest 

        #if there is a clear growth or decay pattern in the dimensions of intersecting models, follow that pattern
        ...
        # if all candidates are different design a scoring option to select the best candidate
        ...
        if count == 1:
            ...
            for candidate in candidates:
                ...
                # Reward higher dimensions when trust-region is small and smaller diimensions when trust-region is large
                if self.delta < self.delta_initial / 4:
                    score += candidate * 0.5
                else:
                    score -= candidate * 0.5
    ...
    def record_gradient_eigenvalues(self, gradients: np.ndarray) -> None:
        """Compute and store eigenvalues from gradient outer product for dimension estimation.

        Args:
            gradients: Array of shape (M, n) containing M gradient samples in n dimensions.
        """
        try:
    ...
    def detect_plateau(self) -> bool:
        """
            ...
            A plateau is defined by a lack of significant improvement in the algorithm for an extended perio
        ...
        no_improvement = all(abs(recent[i] - recent[0]) < 1e-10 for i in range(1, len(recent)))
        ...
        is_plateau = (
            (relative_improvement < self.plateau_threshold and require_consecutive_failures)
        ...
        if is_plateau:
            logging.info(
                ...
                f"relative_improvement={relative_improvement:.6f} < threshold={self.plateau_threshold}, "
    ...
    def reduce_dimension_in_plateau_reset(self) -> int: 
        """
            A deterministic decay function to reduce the subspace dimension during a plateau reset cycle.
            The dimension is reduced gradually back to the initial dimension over a set number of iterations.

             The decay follows: d(t) = d_max - (d_max - d_init) * log(1 + t) / log(1 + T)
             where t is the number of iterations since the reset started, and T is the decay duration.
        Returns:
            int: The subspace dimension for the current iteration during the reset cycle.
        """
        if not self.in_dimension_reset:
            return self.d
        ...
        current_d = max(target_d, min(self.max_d, current_d))
    ...
    def compute_trust_region(self, U: np.ndarray) -> tuple[callable, np.ndarray]: 
        """
            Constructs the ellipsoidal trust-region based on the Hessian of the previous model.
            Uses the stored prev_H from the previous iteration to define the trust-region geometry.
        ...
        H_full = eigvecs @ np.diag(eigvals) @ eigvecs.T

        def trust_region_constraint(x):
            x = np.asarray(x).reshape(-1, 1)
            if x.shape[0] == U.shape[1]:
                H_reduced = U.T @ H_full @ U
                H = 0.5 * (H_reduced + H_reduced.T) 
            else : 
                H = H_full
            return (x.T @ H @ x).item()
    ...
    def solve_subproblem(
        ...
        x_current_reduced = U.T @ x_current  # shape (d, 1)
        ...
        tr_cons, H_full = self.compute_trust_region(U)
        if not np.all(np.isfinite(H_full)):
            H_full = np.eye(self.problem.dim)
        H = U.T @ H_full @ U  # Project Hessian to reduced space
        ...
        if norm(grad_at_zero) < 1e-6 and self.factors["Record Diagnostics"]:
            warning = "⚠️ WARNING: Model gradient is very small - model may be too flat!\n"
    ...
    def update_parameters(
        ...
        """
        current_iteration_performance = lambda success : self.record_iteration_performance(self.delta, self.incumbent_solution, success)
        ...
        if cautious_accept:
            ...
            # This prevents the estimate from changing if the solution gets more samples later
    ...
    def initial_evaluation(self) -> None : 
        """
            Perform the initial evaluation of the incumbent solution with adaptive sampling.
        """
    ...
    def evaluate_interpolation_points(
        self, visited_index: int, X: np.ndarray, delta: float
        ...
        for idx in range(X.shape[0]):
            ...
            if not (idx == 0 and self.iteration_count == 0): 
                adapt_soln = self.perform_adaptive_sampling(adapt_soln, pilot_run, delta)
                budget_total = self.budget.used 
            
            fX.append(
                -1 * self.problem.minmax[0] * adapt_soln.objectives_mean.item()
            )
    ...
    def simulate_candidate_soln(
        ...
        Returns:
                tuple[Solution, float]: The updated candidate solution with simulation results,
        ...
        if self.factors["crn_across_solns"]:
            ...
            pilot_run = self.calculate_pilot_run(construct_model=True)
            candidate_solution = self.perform_adaptive_sampling(candidate_solution, pilot_run, delta)
        
        fval_tilde = -1 * self.problem.minmax[0] * candidate_solution.objectives_mean.item()
    ...
    def compute_adaptive_interpolation_radius_fraction(self) -> list[float]:
        """Compute the semi-axes lengths of the ellipsoidal trust-region in the active subspace.
                The lengths are proportional to the square roots of the eigenvalues of the Hessian of the
                surrogate model projected onto the active subspace.

        Returns:
                list[float]: The list of semi-axes lengths for each dimension in the active subspace
        """
        ...
        H = self.prev_H if self.factors["elliptical trust region"] else np.eye(self.problem.dim)
    ...
    def interpolation_points_without_reuse(self, U: np.ndarray) -> list[np.ndarray]:
        ...
        for i in range(0, self.d):
            ...
            if sum(x_k) != 0:
                ...
                if (
                    degeneration_check(minus, x_k, dup_tol) or 
                    duplication_check(minus, Y, dup_tol)
                ) : 
                    minus = backoff_step(
                        x_k, direction, interpolation_radii[i], lower_bounds, upper_bounds
                    )
                ...
                if (
                    degeneration_check(plus, x_k, dup_tol) or 
                    duplication_check(plus, Y, dup_tol)
                ) : 
                    plus = backoff_step(
                        x_k, direction, interpolation_radii[i], lower_bounds, upper_bounds
                    )
    ...
    def get_rotated_basis(self, A_k1: np.ndarray, U: np.ndarray) -> list[np.ndarray]:
        ...
        # Generate candidate vectors from the FULL standard basis (not just indices 1 to d-1)
        # This ensures we have enough candidates even if some are nearly parallel to A_k1
        I = np.eye(self.d)
        ...
        # Safety check: if we still don't have d vectors (shouldn't happen with full candidates),
        # pad with random orthogonal vectors
        while len(basis) < self.d:
            logging.warning(
                f"get_rotated_basis: Could only generate {len(basis)} of {self.d} basis vectors. "
            ...
            if v_norm > 1e-12:
                ...
                # Extremely rare edge case - use coordinate vector with small perturbation
    ...
    def interpolation_points_with_reuse(
        self, reused_x: np.ndarray, rotation_vectors: list[np.ndarray], U: np.ndarray
    ) -> list[np.ndarray]:
        ...
        if len(rotation_vectors) < self.d:
            logging.error(
                ...
                f"got {len(rotation_vectors)}. Falling back to interpolation without reuse."
            )
    ...
    def construct_interpolation_set(
        ...
        if (self.iteration_count == 1) or not reuse_possible:
            Y = self.interpolation_points_without_reuse(U)
        ...
        # Else if we will reuse one design point
            ...
            Y = self.interpolation_points_with_reuse(reused_pt, rotate_matrix, U)
        return np.vstack([v.ravel() for v in Y]), f_index
    
    def _find_best_reuse_candidate(
        self, U: np.ndarray, x_k: np.ndarray
    ) -> tuple[int, bool]:
        ...
        may project to different locations in different subspaces. By checking full-space
        ...
        For poisedness: A point that projects far from the center in the current subspace
        ...
        H_full = self.prev_H if self.factors["elliptical trust region"] else np.eye(self.problem.dim).reshape(self.problem.dim, self.problem.dim)
        ...
        # - Poisedness quality (98% with Λ ratio ≤ 1.5): Design sets remain well-conditioned
        # Monte Carlo testing showed: 0.1δ → 32% pass, 0.45δ → 98% pass, 0.5δ → 100% pass
    ...
    def generate_set(self, num: int, delta: float | None = None) -> np.ndarray:
        ...
        S = np.zeros((num, self.problem.dim))
        S[0, :] = x_k.flatten()
    ...
    def random_directions(
        ...
        for i in range(nactive):
            ...
            direcs[:, ninactive + i] = (
                self.get_scale(direcs[:, ninactive + i], lower, upper) * direcs[:, ninactive + i]
            )
            ...
            direcs[:, n + ninactive + i] = (
                self.get_scale(direcs[:, n + ninactive + i], lower, upper, 1.0) * direcs[:, n + ninactive + i]
            )
    ...
    def improve_geometry(
        ...
        if max(norm(X_projected - x_k_projected, axis=1, ord=2)) > dist:
            X, fX, interpolation_solutions = self.sample_set(
    ...
    def sample_set(
        ...
        if (
            ...
            X_candidates, fX_candidates = self.remove_furthest_point_projected(
                X_candidates, fX_candidates, x_k, U
            )

        # Remove center point from candidates (will be added as X_new[0])
        X_candidates, fX_candidates = self.remove_point_from_set(
        ...
        X_additional = self.generate_set_in_subspace(delta, U, num_additional)
        # Remove center from additional candidates
        X_additional, _ = self.remove_point_from_set(
        ...
        m = 2 * d + 1
        X_new = np.zeros((m, self.problem.dim))
        fX_new = np.zeros((m, 1))
        X_new[0, :] = x_k.flatten()
        fX_new[0, :] = current_f

        # LU pivoting will select best m-1 points from candidates
        X_new, fX_new, interpolation_solutions = self.LU_pivoting(
            delta,
            X_new,
            fX_new,
            X_all_candidates,
    ...
    def LU_pivoting(
        ...
        X: np.ndarray,
        fX: np.ndarray,
        X_improved: np.ndarray,
        fX_improved: np.ndarray,
        U: np.ndarray,
        interpolation_solutions: list[Solution],
    ) -> tuple[np.ndarray, np.ndarray, list[Solution]]:
        ...
        for k in range(1, max_k):
            ...
            # If index doesn't exist, solve an optimisation problem to find the point in the range which best satisfies criterion
                ...
                if fX_improved.size > 0 and M[index] >= abs(np.dot(v, phi_function(x))):
                    ...
                    pilot_run = self.calculate_pilot_run(construct_model=True)
                    soln_at_x = self.perform_adaptive_sampling(soln_at_x, pilot_run, delta)
                    f_value = (
            ...
            if abs(R[k, k]) < min_pivot:
                ...
                try:
                    ...
                    if abs(R_backup) > abs(R[k, k]):
                        ...
                        pilot_run = self.calculate_pilot_run(construct_model=True)
                        soln_at_s = self.perform_adaptive_sampling(soln_at_s, pilot_run, delta)
                        f_value = (
                            -1
    ...
    def getTotalOrderBasisRecursion(
    ...
    def get_basis(self, orders: np.ndarray) -> np.ndarray:
        ...
        if int(1e6) <= L:
            raise Exception(
                "Cardinality %.1e is >= hard cardinality limit %.1e" % (L, int(1e6))
            )
        # Generate basis
    ...
    def generate_set_in_subspace(
        self, delta: float, U: np.ndarray, num: int
    ) -> np.ndarray:
    ...
    def find_new_point_alternative(
        ...
        X_tmp = self.generate_set_in_subspace(delta, U, no_pts)
    ...
    def remove_point_from_set(
        self, X: np.ndarray, fX: np.ndarray, x: np.ndarray
    ...
    def remove_furthest_point_projected(
        self, X: np.ndarray, fX: np.ndarray, x: np.ndarray, U: np.ndarray
    ) -> tuple[np.ndarray, np.ndarray]:
    ...
    def construct_model(
        ...
        if self.prev_U is not None and self.prev_U.shape == (self.problem.dim, self.d):
            ...
            init_S_full = self.generate_set(self.d)
            U, _ = np.linalg.qr(init_S_full.T)

        X, f_index = self.construct_interpolation_set(U)

        fX, interpolation_solutions = self.evaluate_interpolation_points(f_index, X, self.delta)

        fval = fX.flatten().tolist()

        U, X, fX, interpolation_solutions = self.fit(
            X, fX, interpolation_solutions, U
        )
    ...
    def model_evaluate(
        self, x_proj: np.ndarray, coef: np.ndarray, U: np.ndarray
    ) -> float:
    ...
    def fit(
        ...
        if self.factors['elliptical trust region']:
            self.prev_H = self.model_hess(x_k, full_space=True).reshape(self.problem.dim, self.problem.dim)
        else : 
            self.prev_H = np.eye(self.problem.dim).reshape(self.problem.dim, self.problem.dim)
        ...
        if self.factors.get("adaptive subspace dimension", False) and X.shape[0] >= 2:
            try:
                # If model_grad supports batch evaluation, use it; otherwise compute per-point
                try:
                    full_space_grads = self.model_grad(X, full_space=True)
                except Exception:
                    full_space_grads = np.vstack(
                        [self.model_grad(x.reshape(1, -1), full_space=True).flatten() for x in X]
                    )
                # Ensure shape
        ...
        try:
            ...
            for test_d in range(1, max_test_d + 1):
                ...
                err = np.sqrt(np.mean((fX.reshape(-1,) - preds.reshape(-1,)) ** 2))
        ...
        while True:
            ...
            if gnorm <= tol:
                if not self.fully_linear_test(X, fX, coef, U, kappa_f, kappa_g):
                    X, fX, interpolation_solutions = self.improve_geometry(
    ...
    def converged_subspace_check(
        self, prev_U: np.ndarray, U: np.ndarray, tol: float
    ) -> bool:
        ...
        try:
            ...
            logging.warning("SVD failed in subspace convergence check with error: %s", e)
            try:
                CtC = C.T @ C
        ...
        subspace_dist = (np.max(sin_theta)).item()  # operator norm of projector difference
    ...
    def fit_coef(self, X: np.ndarray, fX: np.ndarray, U: np.ndarray) -> np.ndarray:
        ...
        V_scaled = V_matrix / col_norms_safe

        try:
            # SVD-based pseudo-inverse with truncation for numerical stability
            U_s, svals, Vh = scipy.linalg.svd(V_scaled, full_matrices=False)
    ...
    def grassmann_trajectory(
        self, U: np.ndarray, Delta: np.ndarray, t: float
    ) -> np.ndarray:
        ...
        UZ = np.dot(U, ZT.T)
        U_new = np.dot(UZ, np.diag(np.cos(sig * t))) + np.dot(
    ...
    def jacobian(self, X: np.ndarray, fX: np.ndarray, U: np.ndarray) -> np.ndarray:
        ...
        try:
            Y, sig, ZT = scipy.linalg.svd(V_matrix, full_matrices=False, lapack_driver="gesvd")
        except Exception as e:
    ...
    def gauss_newton_solver(
        self, residual: callable, jacobian: callable, U: np.ndarray, gn_solver: callable
    ) -> np.ndarray:
    ...
    def backtracking(
        self, residual: callable, Grad: np.ndarray, delta: np.ndarray, U: np.ndarray
    ...
    def rotate_U(
        self, X: np.ndarray, fX: np.ndarray, coef: np.ndarray, U: np.ndarray
    ) -> np.ndarray:
    ...
    def grad(self, X: np.ndarray, coef: np.ndarray, U: np.ndarray) -> np.ndarray:
        ...
        Returns:
                np.ndarray: The gradients of the model at each design point X shape (M,d) or (M,n)
        """
    ...
    def hess(self, X: np.ndarray, coef: np.ndarray, U: np.ndarray) -> np.ndarray:
        """Computes the Hessian of the local model at each design point of the design set X
        Args:
                X (np.ndarray): design set of shape (M,n) or (M,d)
                coef (np.ndarray): The coefficients of the local model of shape (q,1)
                U (np.ndarray): The active subspace of shape (n,d)
        ...
        Returns:
                np.ndarray: The Hessians of the model at each design point X of shape (M,d,d)
        """
        if len(X.shape) == 1 or X.shape[0] == 1:
            one_d = True
            X = X.reshape(1, -1)
        ...
        DDV_matrix = self.DDV(Y)  # shape (M,q,d,d)
        # Compute Hessian on projected space by contracting with coefficients
        # DDV_matrix has shape (M,q,d,d), coef has shape (q,1)
        Hf = np.tensordot(DDV_matrix, coef, axes=(1, 0))  # shape (M,d,d,1)
        Hf = np.squeeze(Hf, axis=-1)  # shape (M,d,d)
    ...
    def V(self, X: np.ndarray) -> np.ndarray:
        ...
        V_coordinate = [self.vander(Xs[:, k], self.degree) for k in range(d)]

        V = np.ones((M, q), dtype=X.dtype)

        for k in range(d) : 
            Vk = V_coordinate[k][:,indices[:,k]]  # shape (M,q)
            V *= Vk  # element-wise multiplication broadcasting over M
    ...
    def DV(self, X: np.ndarray) -> np.ndarray:
        ...
        V_deriv = [V_coordinate[k][:, 0:-1] @ Dmat.T for k in range(d)]
        ...
        DV = np.ones((M, q, d), dtype=X.dtype)
        ...
        for k in range(d):
            ...
            DV[:, :, k] = base * ratio  # element-wise multiplication broadcasting over M
    ...
    def DDV(self, X: np.ndarray) -> np.ndarray:
        ...
        Returns:
                np.ndarray: Second derivative of Vandermonde matrix of shape (M,q,d,d) where 
        ...
        V1 = [V_coordinate[k][:, 0:-1] @ Dmat.T for k in range(d)]
        V2 = []
        ...
        for k in range(d) :
            ...
            V2_k = V1_full[:, 0:-1].dot(Dmat)  # second derivative shape (M,degree)
            V2.append(V2_k)

        base = np.ones((M, q), dtype=Xs.dtype)
        for m in range(d):
            Vm = V_coordinate[m][:, indices[:, m]]  # shape (M,q)
            base *= Vm  # element-wise multiplication broadcasting over M
        

        DDV = np.ones((M, q, d, d), dtype=Xs.dtype)
        ...
        for k in range(d):
            ...
            for l in range(d) : 
                ...
                DDV[:, :, k, l] = base * ratio1 * ratio_l * dscale[k] * dscale[l]  # off-diagonal terms
    ...
    def index_set(self, n: int, d: int) -> np.ndarray:
        ...
        if cache_key in ASTROMORF._index_cache:
            return ASTROMORF._index_cache[cache_key].copy()  # Return copy to prevent mutation
    ...
    def index_set_diagonal(self, n: int, d: int) -> np.ndarray:
        ...
        - Diagonal quadratic terms: [2, 0, ..., 0], [0, 2, 0, ..., 0], ..., [0, ..., 0, 2]
...
def duplication_check(x: np.ndarray, X: np.ndarray, tol: float = 1e-8) -> bool:
    """Check if a point x is a duplicate of any point in the set X within a given tolerance.
    ...
    """
    for xi in X:
        if np.linalg.norm(x - xi) < tol:
            return True
    return False


def degeneration_check(x_new: np.ndarray, incumbent_sol: np.ndarray, tol: float = 1e-8) -> bool:
    ...
    """
    if np.linalg.norm(x_new - incumbent_sol) < tol:
        return True
    return False
...
def backoff_step(
    ...
    Args:
            incumbent_sol (np.ndarray): The incumbent solution point, shape (n,).
            direction (np.ndarray): The direction to move away from the incumbent, shape (n,).

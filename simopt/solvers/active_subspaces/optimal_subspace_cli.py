...
def main(problem_name: str, n_macroreps: int = 5, budget: int = 1000, new_dim: int | None = None):

	# Update problem and model factors based on new_dim
	if new_dim is not None :
		problem_factors = update_problem_factor_dimensions(problem_name, new_dim, budget)
	...
	problem = instantiate_problem(problem_name, problem_fixed_factors=problem_factors, model_fixed_factors=model_factors)
...
if __name__ == "__main__":
	...
	parser.add_argument(
		...
		type=int,
		default=1000,
		help="Budget constraint for the problem."
	)
	...
	main(problem_name=args.problem_name, n_macroreps=args.n_macroreps, budget=args.budget, new_dim=args.new_dim)
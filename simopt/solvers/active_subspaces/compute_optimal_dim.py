...
def _test_single_polynomial_degree(args):
    ...
    try:
        ...
        experiment.post_replicate(n_postreps=50, crn_across_budget=True, crn_across_macroreps=False)
        ...
        if os.path.exists(diag_dir):
            ...
            diag_files = [f for f in os.listdir(diag_dir) 
            ...
            for diag_file in sorted(diag_files)[-n_macroreps:]:
                try:
                    with open(os.path.join(diag_dir, diag_file), 'r') as f:
                        content = f.read()
                        
                        # Look for: "The number of successful iterations was X and the number of unsuccessful iterations was Y"
                        success_match = re.search(r"The number of successful iterations was (\d+) and the number of unsuccessful iterations was (\d+)", content)
                        if success_match:
                        ...
                        r2_matches = re.findall(r"R²\s*\(goodness of fit\):\s*([\d.]+)", content)
                        if r2_matches:
                            r_squared_values.extend([float(r2) for r2 in r2_matches])
                except Exception as e:
                    if verbose:
                        logging.debug(f"Could not parse diagnostics file {diag_file}: {str(e)}")
                    continue
        ...
        if len(success_rates) == 0 or len(r_squared_values) == 0:
            if verbose:
                logging.warning(f"Could not parse diagnostics for degree {degree}, using simplified metrics")
        ...
        if verbose:
            logging.error(f"Error testing degree {degree}: {str(e)}")
        return None
...
def find_best_polynomial_degree(
    ...
    if verbose:
        ...
        logging.info(f"Success weight: {success_weight}, Quality weight: {quality_weight}")
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_degree = {executor.submit(_test_single_polynomial_degree, args): args[0] for args in test_args}
        ...
        for future in as_completed(future_to_degree):
            ...
            try:
                ...
                if verbose:
                    logging.error(f"Exception collecting results for degree {degree}: {str(e)}")
    
    if verbose:
        logging.info(f"\nAll parallel experiments completed")
        logging.info(f"Successfully tested {len(all_statistics)} out of {len(degrees_to_test)} degrees")
    ...
    all_success_rates = np.array([all_statistics[d]['success_rate'] for d in tested_degrees])
    all_r_squared = np.array([all_statistics[d]['mean_r_squared'] for d in tested_degrees])
    all_mean_objectives = np.array([all_statistics[d]['mean_objective'] for d in tested_degrees])
    
    # Replace NaN values with sensible defaults (infs are ok, will be clipped)
    all_success_rates = np.nan_to_num(all_success_rates, nan=0.0, posinf=1.0, neginf=0.0)
    all_r_squared = np.nan_to_num(all_r_squared, nan=0.0, posinf=1.0, neginf=0.0)
    all_mean_objectives = np.nan_to_num(all_mean_objectives, nan=np.inf, posinf=np.inf, neginf=-np.inf)
    ...
    for degree in tested_degrees:
        ...
        mean_obj = np.nan_to_num(mean_obj, nan=np.inf if problem.minmax[0] == -1 else -np.inf)
        ...
        if verbose:
            logging.info(f"\nDegree {degree}:")
            logging.info(f"  Success score: {success_score:.4f} (Rate: {success_rate:.2%})")
    ...
    if verbose:
        ...
        logging.info(f"  Success rate: {all_statistics[optimal_degree]['success_rate']:.2%}")
        logging.info(f"  Mean R²: {all_statistics[optimal_degree]['mean_r_squared']:.4f}")
        logging.info(f"  Mean objective: {all_statistics[optimal_degree]['mean_objective']:.4f}")
        logging.info(f"{'='*60}")
...
def _test_single_dimension(args):
    ...
    try:
        ...
        if verbose:
            logging.error(f"Error testing dimension {dim}: {str(e)}")
        return None  # Return None for failed experiments
...
def find_best_subspace_dimension(
    ...
    if verbose:
        ...
        logging.info(f"Consistency weight: {consistency_weight}, Quality weight: {quality_weight}")
        logging.info(f"Parallelizing experiments across {len(dimensions_to_test)} dimensions")
    ...
    for dim in dimensions_to_test:
        try:
            result = _test_single_dimension((dim, problem, solver_name, n_macroreps, max_dimension, verbose))
            if result is not None:
                # Unpack results
    ...
    if verbose:
        logging.info(f"\nAll parallel experiments completed")
        logging.info(f"Successfully tested {len(all_statistics)} out of {len(dimensions_to_test)} dimensions")
    ...
    for dim in tested_dims:
        ...
        combined_score = consistency_weight * consistency_score + quality_weight * quality_score
        ...
        if verbose:
            logging.info(f"\nDimension {dim}:")
            logging.info(f"  Consistency score: {consistency_score:.4f} (Std: {std_dev:.4f})")
    ...
    if verbose:
        ...
        logging.info(f"  Combined score: {combined_scores[best_idx]:.4f}")
        logging.info(f"  Mean objective: {all_statistics[optimal_dimension]['mean']:.4f}")
        logging.info(f"  Standard deviation: {all_statistics[optimal_dimension]['std']:.4f}")
        logging.info(f"  Coefficient of variation: {all_statistics[optimal_dimension]['cv']:.4f}")
        logging.info(f"{'='*60}")

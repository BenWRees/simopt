...
def check_sample_inputs(X, fX, grads):  # noqa: ANN001, ANN202, N803
    ...
    if grads is not None:
        ...
        if X is not None:
            assert X.shape[1] == grads.shape[1], (
...
class Function(BaseFunction):
    ...
    def __init__(  # noqa: D107
        ...
        if grads is not None:
            if callable(grads):
                grads = [grads]
            assert len(grads) == len(self._funs), (
                "Must provide the same number of functions and gradients"
            )
            self._grads = grads
        else:
            self._grads = None
    ...
    def eval_async(self, X_norm, **kwargs):  # noqa: ANN001, ANN003, ANN201, N803
        ...
        results = [
            self.dask_client.submit(subcall, self._funs_pickle, x, **kwargs) for x in X
        ]
    ...
    def grad(self, X_norm, **kwargs):  # noqa: ANN001, ANN003, ANN201, D102, N803
        ...
        if self._grads is not None:
            ...
            if self.vectorized:
                ...
                grads = np.array(
                    [np.vstack([grad(x, **kwargs) for grad in self._grads]) for x in X]
                )

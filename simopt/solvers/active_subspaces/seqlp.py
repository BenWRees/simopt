...
def sequential_lp(  # noqa: ANN201
    ...
    **kwargs,  # noqa: ANN003
    ...
    assert len(constraints) == len(constraint_grads), (
    ...
    try:
        ...
        fx = np.array([fi(x) for fi in fun]).reshape(
            -1,
        )
    objval = objfun(fx)
    ...
    for it in range(maxiter):
        ...
        for it2 in range(bt_maxiter):
            active_constraints = (
                nonlinear_constraints + domain_constraints + search_step_constraints
            )
            ...
            if (status == "unbounded" or status == "unbounded_inaccurate") and it2 == 0:
                ...
                try:
                    ...
                    fx_new = np.array([fi(x_new) for fi in fun]).reshape(
                        -1,
                    )

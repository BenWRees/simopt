...
class IndexSet:
    ...
    def set_orders(self, orders) -> None:  # noqa: ANN001
        ...
        if name.lower() == "total-order":
            ...
            basis = np.reshape(
                np.linspace(0, self.orders[0], self.orders[0] + 1),
                (self.orders[0] + 1, 1),
            )
    ...
    def get_basis(self):  # noqa: ANN201
        ...
        if name == "total-order":
            ...
            sparse_index, sparse_weight_factors, sparse_grid_set = sparse_grid_basis(
                self.level, self.growth_rule, self.dimensions
            )  # Note sparse grid rule depends on points!
...
# PRIVATE FUNCTIONS
# ---------------------------------------------------------------------------------------------------
def euclidean_degree_basis(orders):  # noqa: ANN001, ANN202
...
def getIndexLocation(small_index, large_index):  # noqa: ANN001, ANN202, N802
    index_values = []
    i = 0
...
def hyperbolic_basis(orders, q):  # noqa: ANN001, ANN202
    ...
    for i in range(0, len(n_bar)):
        ...
        summation[0, i] = (np.sum(array_entry)) ** (
            1.0 / (1.0 * q)
...
def getTotalOrderBasisRecursion(highest_order, dimensions):  # noqa: ANN001, ANN202, N802
    if dimensions == 1:
...
def total_order_basis(orders):  # noqa: ANN001, ANN202
    # init
    dimensions = len(orders)
    highest_order = np.max(orders)
    # Check what the cardinality will be, stop if too large!
    L = int(  # noqa: N806
        mt.factorial(highest_order + dimensions)
        / (mt.factorial(highest_order) * mt.factorial(dimensions))
    )
    # Check cardinality
    ...
    # Generate basis
    # Special-case highest_order == 2 to avoid deep recursion when `dimensions` is large.
    if highest_order == 2:
        # Construct rows: sum = 0, 1, or 2.
        rows = []
        # sum == 0
        rows.append(np.zeros(dimensions, dtype=float))
        # sum == 1 -> one coordinate = 1
        for i in range(dimensions):
            r = np.zeros(dimensions, dtype=float)
            r[i] = 1
            rows.append(r)
        # sum == 2 -> either one coordinate = 2 or two coordinates = 1
        for i in range(dimensions):
            r = np.zeros(dimensions, dtype=float)
            r[i] = 2
            rows.append(r)
...
def sparse_grid_basis(level, growth_rule, dimensions):  # noqa: ANN001, ANN202
    # Initialize a few parameters for the setup
    level_new = level - 1
    lhs = int(level_new) + 1
    ...
    for i in range(0, len(summation2)):
        ...
        n = int(dimensions - 1)
        value = (-1) ** k * (
            mt.factorial(n) / (1.0 * mt.factorial(n - k) * mt.factorial(k))
        )
        a.append(value)
...
def tensor_grid_basis(orders):  # noqa: ANN001, ANN202
...
def column(matrix, i):  # noqa: ANN001, ANN202

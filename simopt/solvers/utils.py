...
def bfgs_hessian_approx(
    ...
    # Get indices where the masks are True
    non_neg_idx = np.where(bounds_non_neg)[0]
    non_pos_idx = np.where(bounds_non_pos)[0]
    x1[non_neg_idx, non_neg_idx] += steps[non_neg_idx]
    x2[non_pos_idx, non_pos_idx] -= steps[non_pos_idx]

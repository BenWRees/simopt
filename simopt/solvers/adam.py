...
class ADAM(Solver):
	...
	def _finite_diff(
		...
		for i in range(problem.dim):
			...
			#Check variable bounds for x1 and x2 that will be simulated
			for i in range(len(x1_tuple)) :
				# Check variable bounds.
				if x1_tuple[i]  > upper_bound[i]:
					x1_tuple[i] = upper_bound[i] - 10**(-7)
				if x1_tuple[i]  < lower_bound[i]:
					x1_tuple[i] = lower_bound[i] + 10**(-7)

			for i in range(len(x2_tuple)) :
				# Check variable bounds.
				if x2_tuple[i]  > upper_bound[i]:
					x1_tuple[i] = upper_bound[i] - 10**(-7)
				if x2_tuple[i]  < lower_bound[i]:
					x2_tuple[i] = lower_bound[i] + 10**(-7)

			x1_solution = self.create_new_solution(tuple(x1_tuple), problem)
			x2_solution = self.create_new_solution(tuple(x2_tuple), problem)

...
class OMoRFConfig(SolverConfig):
    ...
    interpolation_update_tol: Annotated[
        ...
        Field(
            default=(0.01, 0.1),
            description="tolerance values to check for updating the interpolation model",  # noqa: E501
    ...
    gamma_1: Annotated[
        ...
        Field(
            ...
            lt=1,
            gt=0,
            description="trust-region radius increase rate after unsuccessful iteration",  # noqa: E501
        ),
    ]
    gamma_2: Annotated[
        float,
...
class OMoRF(Solver):
    ...
    @U.setter
    def U(self, value: np.ndarray) -> None:  # noqa: N802
    ...
    def _set_delta(self, val: float | np.floating) -> None:  # type: ignore[type-arg]
        """Set the trust region radius."""
        self._delta_k = float(val)
    ...
    def blackbox_evaluation(self, s: np.ndarray, problem: Problem) -> float:
        ...
        if (
            getattr(self, "S", np.array([])).size > 0
            and np.unique(np.vstack((self.S, s)), axis=0).shape[0] == self.S.shape[0]
        ):
    ...
    def calculate_subspace(  # noqa: D417
        self,
        ...
        M, n = S.shape  # noqa: N806
        num_grad_lb = 2.0 * self.d * np.log(n)

        if num_grad_lb > M:
            warnings.warn(
                "Number of gradient evaluation points is likely to be insufficient. Consider resampling!",  # noqa: E501
                UserWarning,
                stacklevel=2,
            )
        ...
        R = covar * weights  # noqa: N806
        C = np.dot(covar.T, R)  # noqa: N806
        ...
        if hasattr(self, "data_scaler"):
            Xmax, Xmin = np.max(S, axis=0), np.min(S, axis=0)  # noqa: N806
        ...
        U0 = subspace[:, 0].reshape(-1, 1)  # this is a column vector  # noqa: N806
        U1 = subspace[:, 1:]  # noqa: N806

        # Add the other d-1 columns to U0 by selecting the columns of U1 with the largest coefficients of determination  # noqa: E501
        for _i in range(self.d - 1):
            ...
            )  # add the column corresponding to the largest coefficient of determination in the orthogonal complement to AS  # noqa: E501
    ...
    def construct_model(self, S, f, poly_basis, U=None) -> list[float]:  # noqa: ANN001, N803
        """Construct the interpolation model by solving the linear solution V(Y)coeff =.
        ...
        M = poly_basis.V(  # noqa: N806
            Y, d
        )  # now constructs M based on the polynomial basis being used
        # print(f'shape of M: {M.shape}')
        return np.matmul(pinv(M), f)

    def eval_model(self, x_k, poly_basis, coeff=None):  # noqa: ANN001, ANN201, D102
        ...
        if (
            poly_dim is not None
            and input_dim != poly_dim
            and (
                hasattr(self, "U")
                and input_dim == getattr(self, "n", input_dim)
                and poly_dim < input_dim
            )
        ):
            interpolation_set = (self.U.T @ interpolation_set.T).T
            input_dim = interpolation_set.shape[1]
            # If dimensions still differ (e.g., basis was constructed for a different reduced  # noqa: E501
        ...
        X = poly_basis.V(interpolation_set, dim_used)  # noqa: N806
        evaluation = X @ coeff
    ...
    def finite_difference_gradient(
        ...
        Args:
                        new_solution (Solution): The solution at which to calculate the
                        gradient.
                        problem (Problem): The problem`that contains the function to
                        differentiate.
        ...
        BdsCheck = np.subtract(forward, backward)  # noqa: N806

        self.budget.request((2 * problem.dim) + 1)
        return finite_diff(self, new_solution, BdsCheck, problem, 0.000000001, 5)
    

    def finite_differencing(  # noqa: ANN201, D102
        ...
        BdsCheck = np.zeros(self.n)  # noqa: N806

        FnPlusMinus = np.zeros((self.n, 3))  # noqa: N806
    ...
    def _get_grads(self, X: np.ndarray, f: np.ndarray, delta_k: float) -> np.ndarray:  # noqa: D417, N803
        """Calculate gradients.
        ...
        Returns:
                        np.ndarray: (N,m) matrix of N gradients evaluated at each row of
                        X
        """
    ...
    def solve(self, problem) -> None:  # noqa: ANN001, D102
        # initialise factors:
        ...
        """ 
		self.s_old = self._apply_scaling(s_old)  # shifts old solution into unit ball
		"""
        ...
        highest_order = 2
        # find largest q such that comb(highest_order+q, highest_order) < CARD_LIMIT_HARD  # noqa: E501
        q_test = 0
        q_max = 0
        while True:
            L = comb(highest_order + q_test, highest_order)  # noqa: N806
        ...
        if q_requested > q_max:
            warnings.warn(
                f"Requested index-set dimension q={q_requested} would produce cardinality >= {CARD_LIMIT_HARD}. Truncating to q={q_max}.",  # noqa: E501
                stacklevel=2,
            )
        ...
        S_full = self.geometry_instance.generate_set(  # noqa: N806
        ...
        S_red, f_red = self.geometry_instance.sample_set(  # noqa: N806
            "new",
            self.s_old,
            self.delta_k,
            self.rho_k,
        ...
        try:
            while self.budget.remaining > 0:
                print(
                    f"\niteration: {self.iteration_count} \t expended budget {self.budget.used} \t current objective function value: {self.f_old}"  # noqa: E501
                )
                ...
                except Exception:  # thrown if the sample set is not defined properly
                ...
                candidate_solution, S_full, S_red, f_full, f_red, reset_flag = (  # noqa: N806
                ...
                S_red, S_full, f_red, f_full = self.evaluate_candidate_solution(  # noqa: N806
                    problem,
                    candidate_fval,
                    candidate_solution,
                    S_red,
    ...
    def solve_subproblem(  # noqa: ANN201
        self,
        problem: Problem,
        S_full: np.ndarray,  # noqa: N803
        S_red: np.ndarray,  # noqa: N803
        f_full: float,
        f_red: float,
    ):
        ...
        def obj(x):  # noqa: ANN001, ANN202
            ...
            return self.eval_model(proj_x.reshape(1, -1), self.poly_basis_model)
        ...
        if step_dist < omega_s * self.rho_k:
            ...
            S_full, f_full, S_red, f_red, delta_k, rho_k, U = (  # noqa: N806
    ...
    def evaluate_candidate_solution(  # noqa: ANN201, D102
        self,
        _problem: Problem,
        fval_tilde: float,
        ...
        print(
            f"DIFFERENCE IN CANDIDATE EVALUATION AT MODEL AND FUNCTION: {abs(fval_tilde - model_eval_new)}"  # noqa: E501
        )

        #! ONLY ISSUE IS THAT IT ACCEPTS GROWING VALUES - DUE TO BIG DIFFERENCE BETWEEN model_eval_new AND fval_tilde  # noqa: E501
        ...
        print(
            f"The model evaluation for the old value is {model_eval_old} and for the candidate value it is {model_eval_new}"  # noqa: E501
        )
        print(
            f"The old function value is {self.f_old} and the new function value is {fval_tilde}"  # noqa: E501
        )
        ...
        #! Need to handle the case where the model evaluation is increasing when it should be decreasing - should reject this!  # noqa: E501
		...
		self.s_old = self.S[ind_min,:] #get the x-val that corresponds to the smallest
		function value
        ...
        S_full, f_full = self.geometry_instance.sample_set(  # noqa: N806
            "replace",
            self.s_old,
            self.delta_k,
            self.rho_k,

...
class Optimisation:
    ...
    def add_nonlinear_ineq_con(self, poly=None, custom=None) -> None:  # noqa: ANN001
        ...
        if poly is not None:
            ...
            # other methods add inequality constraints using dictionary files
                ...
                if not np.isinf(bounds[0]):
                    self.constraints.append(
                        {
                            "type": "ineq",
                            "fun": lambda x: constraint(x) - bounds[0],
                            "jac": constraint_deriv,
                        }
                    )
                if not np.isinf(bounds[1]):
                    self.constraints.append(
                        {
                            "type": "ineq",
                            "fun": lambda x: -constraint(x) + bounds[1],
                ...
                if not np.isinf(bounds[0]):
                    self.constraints.append(
                        {"type": "ineq", "fun": lambda x: constraint(x) - bounds[0]}
                    )
                if not np.isinf(bounds[1]):
                    self.constraints.append(
                        {"type": "ineq", "fun": lambda x: -constraint(x) + bounds[1]}
                    )
        elif custom is not None:
            assert "function" in custom
            ...
            if self.method == "trust-constr":
                ...
                if "jac_function" in custom:
                    self.constraints.append(
                        {"type": "ineq", "fun": constraint, "jac": constraint_deriv}
                    )
                else:
        ...
        if poly is not None:
            ...
            if self.method == "trust-constr":
                ...
                self.constraints.append(
                    {
                        "type": "eq",
                        "fun": lambda x: constraint(x) - value,
                        "jac": constraint_deriv,
                    }
                )
        elif custom is not None:
            ...
            if self.method == "trust-constr":
                self.constraints.append(
                    optimize.NonlinearConstraint(
                        constraint, 0.0, 0.0, jac=constraint_deriv, hess=constraint_hess
                    )
                )
            else:
                if "jac_function" in custom:
                    self.constraints.append(
                        {"type": "eq", "fun": constraint, "jac": constraint_deriv}
                    )
                else:
    ...
    def optimise(self, x0, *args, **kwargs):  # noqa: ANN001, ANN002, ANN003, ANN201, ARG002
        ...
        if self.method in [
            ...
            self._trust_region(
                x0,
                del_k=kwargs.get("del_k"),
                rho_min=kwargs.get("rho_min", 1.0e-6),
                eta_1=kwargs.get("eta_1", 0.1),
                eta_2=kwargs.get("eta_2", 0.7),
                gam_dec=kwargs.get("gam_dec", 0.5),
                gam_inc=kwargs.get("gam_inc", 2.0),
                gam_inc_overline=kwargs.get("gam_inc_overline", 2.5),
                alpha_1=kwargs.get("alpha_1", 0.1),
                alpha_2=kwargs.get("alpha_2", 0.5),
                omega_s=kwargs.get("omega_s", 0.5),
                max_evals=kwargs.get("max_evals", 10000),
            ...
            self._omorf(
                x0,
                d=kwargs.get("d", 1),
                subspace_method=kwargs.get("subspace_method", "active-subspaces"),
                del_k=kwargs.get("del_k"),
                rho_min=kwargs.get("rho_min", 1.0e-8),
                eta_1=kwargs.get("eta_1", 0.1),
                eta_2=kwargs.get("eta_2", 0.7),
                gam_dec=kwargs.get("gam_dec", 0.98),
                gam_inc=kwargs.get("gam_inc", 2.0),
                gam_inc_overline=kwargs.get("gam_inc_overline", 2.5),
                alpha_1=kwargs.get("alpha_1", 0.9),
                alpha_2=kwargs.get("alpha_2", 0.95),
                omega_s=kwargs.get("omega_s", 0.5),
                max_evals=kwargs.get("max_evals", 1000),
                random_initial=kwargs.get("random_initial", False),
                scale_bounds=kwargs.get("scale_bounds", False),
            )
            sol = {"x": self.s_old, "fun": self.f_old, "nfev": self.num_evals}
    ...
    def _set_iterate(self) -> None:
        ...
        self.s_old = self.S[
            ind_min, :
        ]  # get the x-val that corresponds to the smallest function value
    ...
    def _random_directions(self, num_pnts, lower, upper):  # noqa: ANN001, ANN202
        ...
        for i in range(nactive):
            ...
            direcs[:, ninactive + i] = (
                self._get_scale(direcs[:, ninactive + i], self.del_k, lower, upper)
                * direcs[:, ninactive + i]
            )
            ...
            direcs[:, self.n + ninactive + i] = (
                self._get_scale(direcs[:, self.n + ninactive + i], 1.0, lower, upper)
                * direcs[:, self.n + ninactive + i]
            )
    ...
    def _LU_pivoting(self, S, f, S_hat, f_hat, full_space, method=None):  # noqa: ANN001, ANN202, N802, N803
        ...
        for k in range(1, q):
            ...
            #           If index doesn't exist, solve an optimisation problem to find the point in the range which best satisfies criterion
                ...
                try:
                    s = self._find_new_point(
                        v, phi_function, phi_function_deriv, full_space
                    )
        ...
        if self.method == "trust-region":
            ...
            def phi_function(s):  # noqa: ANN001, ANN202
                ...
                for k in range(self.q):
                    phi[:, k] = np.prod(
                        np.divide(
                            np.power(u, self.basis[k, :]), factorial(self.basis[k, :])
                        ),
                        axis=1,
                    )
                if m == 1:
    ...
    def _compute_step(self, my_poly):  # noqa: ANN001, ANN202
        ...
        if self.method == "trust-region":
            res = optimize.minimize(
                lambda x: np.asscalar(my_poly.get_polyfit(x)),
                self.s_old,
                method="TNC",
                jac=lambda x: my_poly.get_polyfit_grad(x).flatten(),
                bounds=bounds,
                options={"disp": False},
            )
        elif self.method == "omorf":
            res = optimize.minimize(
                lambda x: np.asscalar(my_poly.get_polyfit(np.dot(x, self.U))),
                self.s_old,
                method="TNC",
                jac=lambda x: np.dot(
                    self.U, my_poly.get_polyfit_grad(np.dot(x, self.U))
                ).flatten(),
                bounds=bounds,
                options={"disp": False},
            )
        ...
        if del_k is None:
            if self.bounds is None:
                ...
                self._set_del_k(
                    min(
                        0.1
                        * np.linalg.norm(self.bounds[1] - self.bounds[0], ord=np.inf),
                        1.0,
                    )
                )
        else:

...
class Correlations:
    ...
    def __init__(  # noqa: D107
        ...
        self.std = Parameter(
            order=5, distribution="normal", shape_parameter_A=0.0, shape_parameter_B=1.0
        )
        inf_lim = -8.0
        sup_lim = -inf_lim
    ...
    def get_pdf(self, X):  # noqa: ANN001, ANN201, N803
        ...
        marginal_prod = np.prod(
            np.array([parameters[i].get_pdf(X[:, i]) for i in range(d)]), axis=0
        )

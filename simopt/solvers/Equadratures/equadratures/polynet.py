        ...
        for k in range(self.num_layers):
            for j in range(self.num_ridges[k]):
                self.poly_array[k, j] = Poly(
                    Parameter(
                        order=poly_deg, distribution="uniform", lower=-3, upper=3
                    ),
                    Basis("total-order"),
                )
    ...
    def evaluate_fit(self, x, train=False):  # noqa: ANN001, ANN201
        ...
        for k in range(1, self.num_layers):
            ...
            for i in range(self.num_ridges[k - 1]):
                Y[k - 1][i] = np.squeeze(
                    np.array(self.poly_array[k, i].get_polyfit(Z[k - 1][i]))
                )
        ...
        for i in range(self.num_ridges[self.num_layers - 1]):
            y_final[i] = np.squeeze(
                np.array(
                    self.poly_array[self.num_layers - 1, i].get_polyfit(
    ...
    def update_delta(self) -> None:  # noqa: D102
        ...
        for k in range(num_layers):
            for i in range(num_ridges[k]):
                act_mat[k][i] = np.squeeze(
                    np.array(self.poly_array[k, i].get_polyfit_grad(Z[k][i]))
                )
        ...
        for t in range(self.max_iters):
            ...
            for k in range(self.num_layers):
                ...
                if self.opt == "sd":
                    ...
                    W_new[k] = (
                        W_new[k]
                        - self.learning_rate * Wk_grads
                        - self.momentum_rate * W_change[k]
                    )
                elif self.opt == "adapt":
                ...
                coeffs_new[k] = self.coeffs[k] - self.learning_rate * np.sum(
                    alphak_grads, axis=2
                )

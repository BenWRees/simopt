...
class Beta(Distribution):
    ...
    def __init__(self, lower=None, upper=None, shape_A=None, shape_B=None) -> None:  # noqa: ANN001, D107, N803
        ...
        if self.shape_A <= 0 or self.shape_B <= 0:
            raise ValueError(
                "Invalid Beta distribution parameters. Alpha and beta should be positive."
            )
        ...
        self.mean, self.variance, self.skewness, self.kurtosis = beta.stats(
            self.shape_A, self.shape_B, loc=loc, scale=scale, moments="mvsk"
        )
    ...
    def get_description(self):  # noqa: ANN201
        ...
        return (
            ...
            + ", to "
            + str(self.upper)
            + ". It has two shape parameters, given here to be "
            + str(self.shape_A)
            + " and "
            + str(self.shape_B)
            + "."

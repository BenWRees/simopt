...
class Parameter:
    ...
    def __init__(  # noqa: D107
        ...
        if self.endpoints is not None:
            if (
                (self.distribution.bounds[0] == -np.inf)
                and (self.distribution.bounds[1] == np.inf)
                and (self.endpoints.lower() == "both")
            ):
                raise (
                    ValueError,
                    "Parameter: The lower bound for your distribution is -infinity and the upper bound is infinity. Furthermore, you have selected the to have both endpoints. These options are incompatible!",
                )
            if (self.distribution.bounds[0] == -np.inf) and (
                self.endpoints.lower() == "lower"
            ):
                raise (
                    ValueError,
                    "Parameter: The lower bound for your distribution is -infinity and you have selected the lower bound option in the endpoints. These options are incompatible!",
                )
            if (self.distribution.bounds[1] == np.inf) and (
                self.endpoints.lower() == "upper"
            ):
                raise (
                    ValueError,
    ...
    def _set_distribution(self) -> None:
        ...
        if self.name.lower() == "gaussian" or self.name.lower() == "normal":
            ...
            self.distribution = Triangular(
                self.lower, self.upper, self.shape_parameter_A
            )
            ...
            self.distribution = Beta(
                self.lower, self.upper, self.shape_parameter_A, self.shape_parameter_B
            )
        elif self.name.lower() == "truncated-gaussian":
            self.distribution = TruncatedGaussian(
                self.shape_parameter_A, self.shape_parameter_B, self.lower, self.upper
            )
            ...
            self.name.lower() == "students-t"
            or self.name.lower() == "t"
            or self.name.lower() == "studentt"
        ):
    ...
    def _get_orthogonal_polynomial(self, points, order=None, grad=False, hess=False):  # noqa: ANN001, ANN202
        ...
        derivative_orthopoly = (
            np.zeros((order, len(gridPoints))) if (grad or hess) else None
        )
        ...
        if order >= 3:  # CHANGED 2/2/18
            for u in range(2, order):  # CHANGED 2/2/18
                ...
                orthopoly[u, :] = (
                    ((gridPoints - ab[u - 1, 0]) * orthopoly[u - 1, :])
                    - np.sqrt(ab[u - 1, 1]) * orthopoly[u - 2, :]
                ) / (1.0 * np.sqrt(ab[u, 1]))
            if grad or hess:
                for u in range(2, order):  # CHANGED 2/2/18
                    # Four-term recurrence formula for derivatives of orthogonal polynomials!
                    derivative_orthopoly[u, :] = (
                        ((gridPoints - ab[u - 1, 0]) * derivative_orthopoly[u - 1, :])
                        - (np.sqrt(ab[u - 1, 1]) * derivative_orthopoly[u - 2, :])
                        + orthopoly[u - 1, :]
                    ) / (1.0 * np.sqrt(ab[u, 1]))
            if hess:
                for u in range(2, order):
                    # Four-term recurrence formula for second derivatives of orthogonal polynomials!
                    dderivative_orthopoly[u, :] = (
                        ((gridPoints - ab[u - 1, 0]) * dderivative_orthopoly[u - 1, :])
                        - (np.sqrt(ab[u - 1, 1]) * dderivative_orthopoly[u - 2, :])
                        + 2.0 * derivative_orthopoly[u - 1, :]
                    ) / (1.0 * np.sqrt(ab[u, 1]))
    ...
    def _get_local_quadrature(self, order=None, ab=None):  # noqa: ANN001, ANN202
        ...
        if self.endpoints.lower() == "lower" or self.endpoints.lower() == "upper":
            return get_local_quadrature_radau(self, order, ab)
        if self.endpoints.lower() == "both":
...
def get_local_quadrature(self, order=None, ab=None):  # noqa: ANN001, ANN201, D103
    ...
    if order == 1:
        ...
        if not self.lower or not self.upper:
            ...
            p = np.asarray((self.upper - self.lower) / (2.0) + self.lower).reshape(
                (1, 1)
            )
        w = [1.0]
    else:
        ...
        for u in range(0, len(idx)):
            w[u] = float(ab[0, 1]) * (
                eigVecs[0, idx[u]] ** 2
            )  # replace weights with right value
            p[u, 0] = eigs[u]

...
def load_eq_dataset(dataset, data_dir=None, verbose=True):  # noqa: ANN001, ANN201
    ...
    if dataset not in datasets:
        raise ValueError(
            "dataset specified in load_eq_dataset not recognised, avaiable datasets: ",
            datasets,
        )
    ...
    if data_dir is None:
        ...
        git_url = posixpath.join(
            "https://github.com/Effective-Quadratures/data-sets/raw/main/",
            dataset,
            dataset + ".npz",
        )
        try:
...
def score(y_true, y_pred, metric="r2", X=None):  # noqa: ANN001, ANN201, N803
    ...
    if metric == "r2":
        ...
        raise ValueError(
            "Only r2, adjusted_r2, mae, normalised_mae, rmse scoring metrics currently supported"
        )
    return score

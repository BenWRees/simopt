...
class GrossModel(Model):
	...
	def function_to_eval(self, x: tuple) -> float :
		sum_x = sum(list(x))
		numer = np.cos(sum_x + np.pi)
		denom = 1 + sum_x 
		res = numer/denom 
		return res 
...
class GrossProblem(Problem) :
	...
	def __init__(self, name: str ="GROSS-1", fixed_factors: dict | None =None, model_fixed_factors: dict | None =None) -> None:
		super().__init__(name=name, fixed_factors=fixed_factors, model_fixed_factors=model_fixed_factors, model=GrossModel)
	...
	def factor_dict_to_vector(self, factor_dict: dict) -> tuple:
		...
		vector : tuple
			vector of values associated with decision variables
		"""
		print(f'type of factor_dict["x"]: {type(factor_dict["x"])}')

...
class DemandInputModel(InputModel):
    ...
    _cached_chol: np.ndarray | None = None
    ...
    def set_cholesky(self, cov: np.ndarray) -> None:
        ...
        """
        self._cached_chol = np.linalg.cholesky(cov)

    def _mvnormalvariate(
        self,
        mean_vec: np.ndarray,
    ) -> np.ndarray:
        """Generate a multivariate normal sample using cached Cholesky factor."""
        if self._cached_chol is None:
            raise RuntimeError("Cholesky factor not set. Call set_cholesky() first.")
        assert self.rng is not None
        n = len(self._cached_chol)
...
class FacilitySize(Model):
    ...
    def before_replicate(self, rng_list: list[MRG32k3a]) -> None:  # noqa: D102
        ...
        # Pre-compute Cholesky decomposition once (not every replication)
        if self._mean_vec is None:
            self._mean_vec = np.array(self.factors["mean_vec"])
            cov = np.array(self.factors["cov"])
            self.demand_model.set_cholesky(cov)

...
class _DiagnosticsBudgetLike(Protocol):
    remaining: int | float
    used: int | float


class _DiagnosticsSolverLike(Protocol):
    name: str
    budget: _DiagnosticsBudgetLike
    d: int
    degree: int
    _polynomial_terms: int
    _conditioning_ratio: float
    iteration_count: int
    successful_iterations: list[object]
    unsuccessful_iterations: list[object]
    incumbent_solution: Solution | None
    incumbent_x: object
    delta: float
    problem: Problem

    def generate_set(self, n_pts: int) -> np.ndarray: ...
...
_ModelCallable = Callable[..., object]
_ModelGradCallable = Callable[..., object]


class ASTROMoRF_Diagnostics:  # noqa: D101, N801
    def __init__(self, solver: object, problem: Problem) -> None:  # noqa: D107
        self.solver = cast(_DiagnosticsSolverLike, solver)
        self.problem = problem
        self.prediction_error_history = []
        self.prev_r_squared = None  # For tracking R² trend over iterations
        self.initialize_diagnostics_file()

    def poisedness_score(self, X, normalize=True):  # noqa: ANN001, ANN201, N803
        ...
        # Normalize by “diameter’’ (max pairwise distance)  # noqa: RUF003
        max_dist = np.max(dists)
        score = 0.0 if max_dist == 0 else min_dist / max_dist

        return score, min_dist

    def initialize_diagnostics_file(self) -> None:  # noqa: D102
        # Create diagnostics file path if it doesn't exist
        # Use timestamp + process ID + random component for uniqueness across parallel processes  # noqa: E501
        self._timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        self._unique_id = (
            f"{self._timestamp}_pid{os.getpid()}_r{random.randint(1000, 9999)}"
        )
        ...
        solver_problem_dir = (
            diagnostics_dir / f"{self.solver.name}_on_{self.problem.name}"
        )
        solver_problem_dir.mkdir(parents=True, exist_ok=True)

        self.diagnostics_file_path = str(
            solver_problem_dir
            / f"astromorf_diagnostics_{self.problem.name}_{self._unique_id}.txt"
        )

        # Check if file is empty and write header if needed
        file_is_empty = (
            not os.path.exists(self.diagnostics_file_path)  # noqa: PTH110
            or os.path.getsize(self.diagnostics_file_path) == 0  # noqa: PTH202
        )

        if file_is_empty and hasattr(self, "_timestamp"):
            with open(self.diagnostics_file_path, "w", encoding="utf-8") as f:  # noqa: PTH123
    ...
    def write_diagnostics_to_txt(self, content: str, mode: str = "a") -> None:
        ...
        """
        with open(self.diagnostics_file_path, mode, encoding="utf-8") as f:  # noqa: PTH123
            f.write(content)
    ...
    def check_polynomial_complexity(self) -> None:
        ...
        if self.solver._conditioning_ratio < 0.3:
            warning_msg = (
                ...
                f"for {q} polynomial terms (ratio={self.solver._conditioning_ratio:.2f}). "  # noqa: E501
                f"This may lead to ill-conditioned systems. Consider reducing "
                f"polynomial degree or increasing subspace dimension.\n"
            )
            self.write_diagnostics_to_txt(warning_msg)

    def write_final_diagnostics(self) -> None:
        """Write final prediction error analysis and solver summary to diagnostics.
        ...
        total_iterations = len(self.solver.successful_iterations) + len(
            self.solver.unsuccessful_iterations
        )
        ...
        if (
            hasattr(self.solver, "incumbent_solution")
            and self.solver.incumbent_solution is not None
        ):
        ...
        print(
            f"\n\u2713 ASTROMoRF Solver completed. Diagnostics written to: {self.diagnostics_file_path}"  # noqa: E501
        )

    def diagnose_candidate_solution(
        self,
        candidate_solution: Solution,
        arg2,  # noqa: ANN001
        arg3=None,  # noqa: ANN001
    ) -> None:
        """Diagnose candidate solution quality by comparing to design points and model.
        ...
        Args:
                ...
                2) (candidate_solution, X)  # model will be taken from self.solver.model
                if available
        ...
        # Distance metrics (computed here for diagnostics, also computed in update_parameters)  # noqa: E501
        distances_to_design = [
        ...
        incumbent_solution = cast(Solution, self.solver.incumbent_solution)
        current_value = (
            -1 * self.problem.minmax[0] * incumbent_solution.objectives_mean.item()
        )
        ...
        diagnostics = {
            ...
            "step_to_radius_ratio": full_space_step / self.solver.delta
            if self.solver.delta > 0
            else 0,
    ...
    def _print_candidate_diagnostics(self, diagnostics: dict) -> None:
        ...
        if diagnostics["relative_error"] > 0.15:
            ...
            if (
                diagnostics["min_dist_to_design"]
                > 0.5 * diagnostics["trust_region_radius"]
            ):
                output += (
                    "  → Candidate is far from design points (interpolation issue)\n"
                )
            ...
            if abs(diagnostics["predicted_improvement"]) < 1e-4:
                output += (
                    "  → Model predicts very small change (model may be too flat)\n"
                )
    ...
    def diagnose_model_quality(
        self,
        model: _ModelCallable,
        model_grad: _ModelGradCallable,
        X: np.ndarray,  # noqa: N803
        fX: np.ndarray,  # noqa: N803, ARG002
        U: np.ndarray,  # noqa: N803
    ) -> None:
        """Compute comprehensive model quality diagnostics to understand poor success.
        ...
        X_test = self.solver.generate_set(no_pts)  # noqa: N806

        # create solutions for each point in the validation set, simulate each point and get true function values  # noqa: E501
        fX_test = np.zeros((no_pts,))  # noqa: N806
        ...
        for i in range(no_pts):
            try:
                ...
                predictions[i] = float(np.asarray(pred_raw).reshape(-1)[0])
            except Exception:
                predictions[i] = np.nan
        fX_test.flatten() - predictions
        ...
        if np.sum(valid) >= 2:
            ...
            ss_tot = np.nansum(
                (fX_test.flatten()[valid] - np.mean(fX_test.flatten()[valid])) ** 2
            )
        ...
        variance_captured = (
            subspace_variance / full_space_variance
            if full_space_variance > 1e-12
            else 0.0
        )
        ...
        orthonormality_error = np.max(
            np.abs(should_be_identity - np.eye(self.solver.d))
        )

        # 3. Design set geometry
        # Compute condition number of Vandermonde matrix
        V_matrix = self.solver.V(X_proj)  # noqa: N806
        cond_V = np.linalg.cond(V_matrix)  # noqa: N806

        # Compute pairwise distances in the projected space (sample 100 pairs if M is large)  # noqa: E501
        sample_size = min(M, 100)
        indices = (
            np.random.choice(M, size=sample_size, replace=False)
            if M > 100
            else np.arange(M)
        )
        distances = []
        ...
        poisedness_score, _min_dist_subspace = self.poisedness_score(
            X_proj, normalize=True
        )
    ...
    def _print_model_diagnostics(self, diagnostics: dict) -> None:
        ...
        if self.prev_r_squared is not None:
            ...
            trend_arrow = (
                "📈" if r2_change > 0.05 else "📉" if r2_change < -0.05 else "→"
            )
        ...
        output += (
            f"  Min point distance:       {diagnostics['min_point_distance']:.4e}\n"
        )
        output += (
            f"  Max point distance:       {diagnostics['max_point_distance']:.4e}\n"
        )
        output += (
            f"  Mean point distance:      {diagnostics['mean_point_distance']:.4e}\n"
        )
        ...
        if diagnostics["variance_captured"] < 0.5:
            issues.append(
                "  ⚠️  Low variance captured - active subspace may be missing important directions"  # noqa: E501
            )
        if diagnostics["vandermonde_condition"] > 1e10:
            issues.append(
                "  ⚠️  Ill-conditioned Vandermonde - design points may be too close or poorly distributed"  # noqa: E501
            )
        if diagnostics["min_point_distance"] < 1e-8:
            issues.append(
                "  ⚠️  Points too close together - numerical instability likely"
            )
        if diagnostics["orthonormality_error"] > 1e-6:
            issues.append(
                "  ⚠️  Active subspace not orthonormal - projection may be inaccurate"
            )

...
class ASTROMoRF_Diagnostics : 
	...
	def diagnose_candidate_solution(self, candidate_solution: Solution, model: callable, X: np.ndarray) :
		"""
	...
	def diagnose_model_quality(self, model: callable, model_grad: callable, X: np.ndarray, fX: np.ndarray, U: np.ndarray) :
		...
		X_test = self.solver.generate_set(self.problem, no_pts)
		...
		predictions = np.array([model(X_test[i, :].reshape(-1, 1)) for i in range(no_pts)])
		residuals = fX_test.flatten() - predictions
		
		# Compute R² (coefficient of determination)
		ss_res = np.sum(residuals**2)
		ss_tot = np.sum((fX_test.flatten() - np.mean(fX_test.flatten()))**2)
		r_squared = 1 - (ss_res / ss_tot) if ss_tot > 1e-12 else 0.0
		
		# Root mean squared error
		rmse = np.sqrt(ss_res / no_pts)

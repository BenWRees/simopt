...
class ProblemSolver:
    ...
    def __init__(
        ...
        Args:
                ...
                file_name_path (Path | str, optional): Path to save a pickled
                ProblemSolver
                        object.
                create_pickle (bool, optional): Whether to save the object as a pickle
                file.
        """
        # Default arguments
        ...
        if file_name_path is not None:
            # If the provided path contains a directory component (e.g. 'output_dir/name.pickle')  # noqa: E501
            # prefer resolving relative to the current working directory so user-specified  # noqa: E501
            ...
            if p is not None and (p.is_absolute() or p.parent != Path()):
                ...
                file_name_path = resolve_file_path(
                    file_name_path, directory=EXPERIMENT_DIR
                )
        ...
        self.all_policy_solutions: list[list] | None = None
        ...
        # Initialize problem.
        if isinstance(problem, Problem | MultistageProblem):  # Method #2
        ...
        if isinstance(self.problem, MultistageProblem):
            self.problem.lookahead_solver = self.solver
    ...
    def model_created(self, model: Model | MultistageModel) -> None:
    ...
    def before_replicate(
        self, model: Model | MultistageModel, rng_list: list[MRG32k3a]
    ) -> None:
    ...
    def check_compatibility(self) -> str:
        ...
        if not self.solver.supports_problem(self.problem):
            expected = self.solver.supported_problem_type
            problem_kind = (
                "MultistageProblem"
                if isinstance(self.problem, MultistageProblem)
                else "Problem"
            ...
            error_message = (
                f"Solver supports {expected.display_name()} instances, "
                f"but got {problem_kind}."
            )
    ...
    def run(self, n_macroreps: int, n_jobs: int = -1) -> None:
        ...
        # Store policy solutions if the solver produced them.
        if "policy_solution" in solution_df.columns:
            self.all_policy_solutions = run_solver._to_list(
                solution_df,
                "policy_solution",
            )
        else:
            self.all_policy_solutions = None

        # Store iteration-level data if available
        if iteration_df is not None:
            self.all_iterations = run_solver._iteration_to_list(
                iteration_df, "iteration"
            )
            self.all_budget_history = run_solver._iteration_to_list(
                iteration_df, "budget_history"
            )
            self.all_fn_estimates = run_solver._iteration_to_list(
    ...
    def run_resumable(
        ...
        if checkpoint_path is None:
            ...
            checkpoint_path = resolve_file_path(
                checkpoint_path, directory=EXPERIMENT_DIR
            )
        ...
        if (
            not hasattr(self, "all_intermediate_budgets")
            or self.all_intermediate_budgets is None
        ):
        ...
        def _signal_handler(signum, _frame) -> None:  # noqa: ANN001
            logging.warning(
                f"Received signal {signum}; checkpointing ProblemSolver to {checkpoint_path}."  # noqa: E501
            )
            try:
        ...
        try:
            ...
            while completed < n_macroreps_total and not self._termination_requested:
                ...
                logging.info(
                    f"Running macroreps {completed + 1}–{completed + n_chunk} "  # noqa: RUF001
                ...
                # Extract policy solutions if available
                if "policy_solution" in solution_df.columns:
                    chunk_policy_solutions = run_solver._to_list(
                        solution_df, "policy_solution"
                    )
                else:
                    chunk_policy_solutions = None
                ...
                if chunk_policy_solutions is not None:
                    if self.all_policy_solutions is None:
                        self.all_policy_solutions = []
                    self.all_policy_solutions.extend(chunk_policy_solutions)
    ...
    def post_replicate(
        ...
        Args:
                n_postreps (int): Number of postreplications at each recommended
                solution.
                crn_across_budget (bool, optional): If True, use CRN across solutions
                from
    ...
    def post_replicate_policy(
        ...
        """Run postreplications using the solver's *policy* at each budget checkpoint.

        This is the policy counterpart of :meth:`post_replicate`.  Instead of
        evaluating the fixed decision vector ``x`` via
        ``problem.simulate(Solution(x))``, it evaluates the full-trajectory
        policy via ``problem.simulate_policy(policy_solution)``.

        The results are stored in the **same** attributes as
        ``post_replicate`` (``all_post_replicates``, ``all_est_objectives``,
        etc.) so that :func:`post_normalize_policy` and the standard plotting
        functions work without modification.

        Args:
                n_postreps: Number of postreplications at each checkpoint.
                crn_across_budget: Use CRN across budget checkpoints.
                crn_across_macroreps: Use CRN across macroreplications.

        Raises:
                RuntimeError: If no policy solutions are available (solver did
                        not produce policies).
        """
        if not self.all_policy_solutions:
            raise RuntimeError(
                "No policy solutions available.  The solver must store "
                "policy_solutions during solve() for policy-based "
                "post-replication (e.g. the ADP solver)."
            )

        if n_postreps <= 0:
            raise ValueError("Number of postreplications must be positive.")

        if not isinstance(self.problem, MultistageProblem):
            raise TypeError("post_replicate_policy requires a MultistageProblem.")

        logging.info(
            f"Setting up {n_postreps} policy postreplications for "
            f"{self.n_macroreps} mreps of {self.solver.name} on "
            f"{self.problem.name}."
        )
        ...
        logging.info("Starting policy postreplications")
        results: list[tuple] = Parallel(n_jobs=-1)(
            delayed(self._post_replicate_policy_mrep)(mrep)
            for mrep in range(self.n_macroreps)
        )
        self.all_stoch_constraints = []
        for mrep, post_rep, timing, stoch_constraints in results:
        ...
        # Compute mean estimated objectives.
        self.all_est_objectives = []
        for mrep in range(self.n_macroreps):
            self.all_est_objectives.append(
                np.array(self.all_post_replicates[mrep]).mean(axis=1)
        ...
        runtime = round(time.time() - function_start, 3)
        logging.info(
            f"Finished {self.n_macroreps} policy postreplications in {runtime} seconds."
        )
    ...
    def _post_replicate_policy_mrep(self, mrep: int) -> tuple:
        """Evaluate policies for one macroreplication via simulate_policy.

        Args:
                mrep: Macroreplication index.

        Returns:
                Tuple of (mrep, post_replicates, runtime, stoch_constraints).
        """
        if self.all_policy_solutions is None:
            raise RuntimeError("No policy solutions available.")
        if not isinstance(self.problem, MultistageProblem):
            raise TypeError("_post_replicate_policy_mrep requires a MultistageProblem.")
        ...
        for budget_index in range(len(self.all_intermediate_budgets[mrep])):
            # Get the stored policy solution for this checkpoint.
            policy_sol = self.all_policy_solutions[mrep][budget_index]

            # Create a fresh policy solution so prior replications don't
            # contaminate the results.
            fresh_sol = self.problem.create_policy_solution(
                policy_sol.x,
                policy=policy_sol.policy,
            )
            if self.crn_across_budget:
                fresh_sol.attach_rngs(rng_list=baseline_rngs, copy=True)
            else:
                fresh_sol.attach_rngs(rng_list=baseline_rngs, copy=False)

            self.problem.simulate_policy(
                solution=fresh_sol,
                num_macroreps=self.n_postreps,
            )

            post_replicates.append(list(fresh_sol.objectives[: fresh_sol.n_reps][:, 0]))

            if self._has_stochastic_constraints():
                stoch_constraints.append(fresh_sol.stoch_constraints)

        toc = time.perf_counter()
        runtime = toc - tic
        logging.debug(f"\t{mrep + 1}: Policy postrep finished in {round(runtime, 3)}s")
    ...
    def bootstrap_sample(
        ...
        Args:
                bootstrap_rng (MRG32k3a): Random number generator used for
                bootstrapping.
                normalize (bool, optional): If True, normalize progress curves with
                respect
                ...
                feasibility_two_sided (bool, optional): Whether to award a non-zero
                score
                        to feasible solutions based on the best violation.
                disable_macrorep_bootstrap (bool, optional): Whether to disable
                bootstrap
    ...
    def log_experiment_results(  # noqa: D417
        self, print_solutions: bool = True, file_path: str | None = None
    ) -> None:
        """Creates a readable .txt log file from a problem-solver pair's .pickle file.

        Args:
                print_solutions (bool, optional): If True, include recommended solutions
                in

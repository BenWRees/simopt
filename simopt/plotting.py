...
def model_callable_from_q(
    q: np.ndarray, dim: int
    ...
    def _model(X: np.ndarray) -> np.ndarray:  # noqa: N803
        Xarr = np.asarray(X)  # noqa: N806
        try:
            Xmat = Xarr.reshape(-1, dim)  # noqa: N806
        except Exception:
            # Provide a helpful message when reshape fails
            raise ValueError(  # noqa: B904
                f"Expected input that can be shaped (-1, {dim}); got shape {Xarr.shape}"
            )
...
def plot_1d_model_construction(
    ...
    if x_min is None:
        ptp_x = float(np.ptp(sample_x))
        x_min = float(sample_x.min()) - 0.2 * (ptp_x if ptp_x != 0 else 1)
    if x_max is None:
        ptp_x = float(np.ptp(sample_x))
        x_max = float(sample_x.max()) + 0.2 * (ptp_x if ptp_x != 0 else 1)

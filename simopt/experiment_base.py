...
def estimate_safe_process_count(mem_per_process_gb=0.5):
	available_gb = psutil.virtual_memory().available / (1024 ** 3)
	return max(1, int(available_gb / mem_per_process_gb))



class PostNormalizeException(Exception):
	"""Exception raised when post-normalization fails."""
...
class ProblemSolver:
	...
	def run(self, n_macroreps: int) -> None:
		...
		max_procs = cpu_count()
		# Use the smaller of macroreps or CPUs
		mem_procs = estimate_safe_process_count(mem_per_process_gb=0.5)
		n_processes = min(n_macroreps, max_procs, mem_procs)
	...
	def post_replicate(
		...
		# Use the smaller of macroreps or CPUs
		mem_procs = estimate_safe_process_count(mem_per_process_gb=0.5)
		n_processes = min(n_postreps, max_procs, mem_procs)
	...
	def log_experiment_results_table(self) -> str : 
		"""
			Writes a CSV file with the computation times of each macroreplication, the performance of each solver against the benchmarks of the optimality gap, 
			the number of iterations run, the average sufficient decrease across iterations, 
			and the number of macroreplications considered solved for a particular problem-solver pair. 

			Returns
			-------
			str
				The file path of the CSV file containing the experiment results.
		"""
		if not os.path.exists(EXPERIMENT_DIR):
			os.makedirs(EXPERIMENT_DIR)
		experiment_name = self.solver.name + '_ON_' + self.problem.name + "_experiment_results.txt"
		...
		if self.problem.optimal_value is None :
			# print("No optimal value provided for the problem. Setting to infinity or zero based on minmax.")
			# print(f"Problem minmax: {self.problem.minmax}")
			if self.problem.minmax[0] == 1 : 
				optimal_value = float("150")
			elif self.problem.minmax[0] == -1 :
				optimal_value = float("0")
				
		# print(f"Optimal Value: {optimal_value} for Problem {self.problem.name} and Solver {self.solver.name}.")
		...
		with open(results_filepath, "w") as file:
			...
			file.write("Macroreplication\tTime Taken (s)\tOptimality Gap\tIterations\tAverage Sufficient Decrease\tSolved\n")

			# Write the data for each macroreplication
			for mrep in range(self.n_macroreps):
				time_taken = round(self.timings[mrep], 2)
				
				#compute the optimality gap of the final recommended solution of the macroreplication
				final_recommended_solution = self.all_recommended_xs[mrep][-1]
				optimality_gap = round(
					abs(
						self.all_est_objectives[mrep][-1] - optimal_value
					),
					4,
				)

				iterations = len(self.all_recommended_xs[mrep])
				avg_sufficient_decrease = round(
					np.mean(
						[
							abs(
								self.all_est_objectives[mrep][budget]
								- optimal_value
							)
							for budget in range(len(self.all_intermediate_budgets[mrep]))
						]
				...
				solved = "No" if optimality_gap > 0.2*optimal_value else "Yes"

				file.write(
					f"{mrep + 1}\t{time_taken}\t{optimality_gap}\t{iterations}\t{avg_sufficient_decrease}\t{solved}\n"
				)
	...
	for experiment in experiments:
		...
		for mrep in range(experiment.n_macroreps):
			...
			if initial_opt_gap == 0:
				# PostNormalizeExceptionWarning('Divide by zero during post-normalization (initial_opt_gap is 0).')
				print(

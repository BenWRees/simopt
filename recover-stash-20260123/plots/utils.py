...
def setup_plot(
    ...
    # Plot types that have their own axis setup and shouldn't use normalize logic
    custom_axis_plots = {
        PlotType.FN_ESTIMATES_ALL,
        PlotType.FN_ESTIMATES_MEAN,
        PlotType.BUDGET_HISTORY_ALL,
        PlotType.BUDGET_HISTORY_MEAN,
    }
    # Set up axes and axis labels.
    if plot_type not in feasibility_plots and plot_type not in custom_axis_plots:
        if normalize:
    ...
    elif plot_type in (PlotType.BUDGET_HISTORY_ALL, PlotType.BUDGET_HISTORY_MEAN):
        plt.xlabel("Iteration", size=14)
        plt.ylabel("Cumulative Budget Used", size=14)
        plt.tick_params(axis="both", which="major", labelsize=12)
        if plot_type == PlotType.BUDGET_HISTORY_ALL:
            title = f"{solver_name} on {problem_name}\nBudget History (All Macroreps)"
        else:
            title = f"{solver_name} on {problem_name}\nMean Budget History"
    elif plot_type in (PlotType.FN_ESTIMATES_ALL, PlotType.FN_ESTIMATES_MEAN):
        plt.xlabel("Iteration", size=14)
...
def save_plot(
    ...
    elif plot_type == PlotType.BUDGET_HISTORY_ALL:
        plot_name = "budget_history_all"
    elif plot_type == PlotType.BUDGET_HISTORY_MEAN:
        plot_name = "budget_history_mean"
    elif plot_type == PlotType.FN_ESTIMATES_ALL:
        plot_name = "fn_estimates_all"
    elif plot_type == PlotType.FN_ESTIMATES_MEAN:
        plot_name = "fn_estimates_mean"
    else:

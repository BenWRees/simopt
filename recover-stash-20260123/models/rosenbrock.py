...
class RosenbrockFunction(Model):
	...
	#! Changed to multiplicative noise 
	def rosenbrock_function(self, x: np.ndarray, noise: MRG32k3a) -> float:
		"""Evaluate the Rosenbrock function at point x."""
		stdev = self.factors["variance"]
		noise_term = noise.normalvariate(mu=1, sigma=stdev)
		terms = [] 
		for i in range(1,len(x)) : 
			terms.append(100 * (x[i] - noise_term*x[i-1] ** 2) ** 2 + (1-x[i-1])**2)
		return np.sum(terms)
	...
	def replicate(self) -> tuple[dict, dict]:
		...
		if d >= 2:
			for i in range(1, d):
				...
				grad[i - 1] += -400.0 * n_grad * term_grad * x[i - 1] + 2.0 * (x[i - 1] - 1.0)

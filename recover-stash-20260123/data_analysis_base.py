...
from simopt.experiment_base import ProblemSolver, read_experiment_results
        ...
        with open(file_path) as file : 
            for line in file : 
                ...
                if match : #if we are at the start of a macroreplication, we start collecting data on the next line
                    ...
                    idx = 0
                    continue 
                if capturing == True and line == '' : #if we reach a new line then we stop capturing and continue reading lines
                    df_list.append(df)
                    df = pd.DataFrame()
                    capturing = False
                    continue  

...
class Statistics:
    ...
    def __init__(
        ...
        self.basis = basis
        self.parameters = (
            parameters  # should be a list containing instances of Parameter
        )
        ...
        self._sobol = private_get_all_sobol_indices(
            self.coefficients, self.basis, max_sobol_order
        )
        # Only required when computing skewness and kurtosis.
        if (
            (quadrature_points is None)
            and (quadrature_weights is None)
            and (polynomial_matrix is None)
        ):
            pass
        else:
            ...
            self._skewness = private_get_skewness(
                self.quadrature_weights,
                self._weighted_evals,
                self.basis,
                self._variance,
            )
            self._kurtosis = private_get_kurtosis(
                self.quadrature_weights,
                self._weighted_evals,
                self.basis,
                self._variance,
...
def private_get_all_sobol_indices(coefficients, basis, max_order):
    ...
    for order in range(1, max_order + 1):  # loop over order
        ...
        for i in range(0, basis_entries):  # loop over rows
            ...
            if (
                len(non_zero_entries) == order
            ):  # neglect entries that should actually be zero (what constitutes as zero?)
                combo_index[tuple(non_zero_entries)] = float(
                    combo_index[tuple(non_zero_entries)]
                    + coefficients[i] ** 2 / variance
                )
...
def private_conditional_skewness(
    ...
    for i in range(1, integral1.shape[0]):
        if sum(norm_ind[i]) == order:
            combo_index[norm_ind[i]] = combo_index[norm_ind[i]] + integral1[i] / (
                variance**1.5 * skewness
            )
    valid_indices = []
    ...
    for p in range(1, basis.cardinality):
        for q in range(1, basis.cardinality):
            summed_norm_index = tuple(
                np.logical_or(norm_ind[p], norm_ind[q]).astype(int)
            )
            ...
            combo_index[summed_norm_index] = combo_index[
                summed_norm_index
            ] + 3 * integral2 / (variance**1.5 * skewness)
    ...
    for a in range(len(valid_indices)):
        for b in range(a + 1, len(valid_indices)):
            for c in range(b + 1, len(valid_indices)):
                ...
                summed_norm_index = tuple(
                    np.logical_or(
                        np.logical_or(norm_ind[p], norm_ind[q]), norm_ind[r]
                    ).astype(int)
                )
                if sum(summed_norm_index) != order:
                    continue
                ...
                evals3 = (
                    weighted_evals[p, :] * weighted_evals[q, :] * weighted_evals[r, :]
                )
...
def private_conditional_kurtosis(
    ...
    for p in valid_indices:
        for q in valid_indices:
            ...
            combo_index[summed_norm_index] = combo_index[
                summed_norm_index
            ] + 4 * integral2 / (variance**2 * kurtosis)
    ...
    for a in range(len(valid_indices)):
        for b in range(a + 1, len(valid_indices)):
            ...
            combo_index[summed_norm_index] = combo_index[
                summed_norm_index
            ] + 6 * integral3 / (variance**2 * kurtosis)
    ...
    for a in range(len(valid_indices)):
        for b in range(len(valid_indices)):
            for c in range(b + 1, len(valid_indices)):
                ...
                combo_index[summed_norm_index] = combo_index[
                    summed_norm_index
                ] + 12 * integral4 / (variance**2 * kurtosis)
...
def delta_pqr(rows):
    ...
    for d in range(dimensions):
        ...
        if col_sum == 2:
            if (
                (rows[0][d] == rows[1][d])
                or (rows[1][d] == rows[2][d])
                or (rows[0][d] == rows[2][d])
            ):  # Hack
                pass
            else:
                return True

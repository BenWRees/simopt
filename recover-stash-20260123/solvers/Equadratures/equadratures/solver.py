...
class Solver:
    ...
    def __init__(self, method, solver_args={}):
        ...
        self.opt = "osqp"
        # Parse solver_args
        if "verbose" in self.solver_args:
            self.verbose = solver_args.get("verbose")
        if "optimiser" in self.solver_args:
            self.opt = solver_args.get("optimiser")
    ...
    def select_solver(method, solver_args={}):
        ...
        if (
            method.lower() == "compressed-sensing"
            or method.lower() == "compressive-sensing"
        ):
            return compressed_sensing(solver_args)
        if method.lower() == "numerical-integration":
            return numerical_integration(solver_args)
        if method.lower() == "least-absolute-residual":
            return least_absolute_residual(solver_args)
        if method.lower() == "huber":
            return huber(solver_args)
        if method.lower() == "relevance-vector-machine" or method.lower() == "rvm":
            return rvm(solver_args)
        if method.lower() == "least-squares-with-gradients":
            return constrained_least_squares(solver_args)
        if method.lower() == "elastic-net":
            return elastic_net(solver_args)
        if method.lower() == "custom-solver":
        ...
        if len(args) != 0:
            ...
            if not self.gradient_flag:
                if len(args) == 2:
                    ...
                    raise ValueError(
                        "get_coefficients only accepts two optional arguments for solvers without gradients; numpy.ndarrays containing the matrices A and b."
                    )
            # Gradients
            else:
                if len(args) == 4:
                    ...
                    raise ValueError(
                        "get_coefficients only accepts four optional arguments for solvers without gradients; numpy.ndarrays containing the matrices A, b, C and d."
                    )
        else:
...
class least_squares(Solver):
    ...
    def solve(self, A, b):
        ...
        if self.verbose is True:
            print(
                "The condition number of the matrix is " + str(np.linalg.cond(A)) + "."
            )
...
class compressed_sensing(Solver):
        ...
        for e in range(len(eta)):
            for n in range(5):
                try:
                    ...
                    x_train = compressed_sensing._bp_denoise(
                        A_train, y_train, eta[e], x0=x0
                    )
    ...
    def _bp_denoise(A, b, epsilon, verbose=False, **kwargs):
        ...
        lbiter = int(
            np.max(
                [
                    np.ceil(
                        (np.log(2.0 * N + 1) - np.log(lbtol) - np.log(tau)) / np.log(mu)
                    ),
                    0.0,
                ]
            )
        ...
        for ii in range(lbiter + 1):
            xp, up, ntiter = compressed_sensing._l1qc_newton(
                x,
                u,
                A,
                b,
    ...
    def _l1qc_newton(
        ...
        while not (done):
            ...
            if use_CG:
                dx, cgres, cgiter = compressed_sensing._CG_solve(
                    H11p, w1p, cgmaxiter, cgtol
                )
            else:
                dx = np.linalg.solve(H11p, w1p).flatten()
                cgres = np.linalg.norm(
                    np.dot(H11p, dx).flatten() - w1p.flatten()
                ) / np.linalg.norm(w1p)
                cgiter = -1
            if cgres > 0.5:
                if verbose:
                    ...
                    [
                        ...
                        np.min(
                            np.hstack(
                                [
                                    -fu1[(dx - du) > 0]
                                    / (dx[(dx - du) > 0] - du[(dx - du) > 0]),
                                    -fu2[(-dx - du) > 0]
                                    / (-dx[(-dx - du) > 0] - du[(-dx - du) > 0]),
                                    np.reshape(
                                        (-bqe + np.sqrt(bqe**2 - 4 * aqe * cqe))
                                        / (2.0 * aqe),
                                        (1,),
                                    ),
                                ]
            ...
            while not (suffdec):
                ...
                fp = np.sum(up) - (1.0 / tau) * (
                    np.sum(np.log(-fu1p)) + np.sum(np.log(-fu2p)) + np.log(-fep)
                )
                ...
                if backiter > 32:
                    if verbose:
                        print(
                            "Stuck on backtracking line search, returning previous iterate."
                        )
                    xp = x.copy()
                    up = u.copy()
                    return xp, up, niter
...
class least_absolute_residual(Solver):
    ...
    def solve(self, A, b):
        ...
        if self.opt == "osqp":
            if self.verbose:
                print("Solving using cvxpy with OSQP solver")
            # Define problem
            b = b.squeeze()
            x = cv.Variable(d)
...
class huber(Solver):
    ...
    def solve(self, A, b):
        ...
        if self.opt == "osqp":
            ...
            r = cv.Variable(N)
            s = cv.Variable(N)
            objective = cv.Minimize(0.5 * cv.sum_squares(z) + self.M * cv.sum(r + s))
            constraints = [A @ x - b - z == r - s, r >= 0, s >= 0]
        ...
        # Use scipy linprog for optimising
            ...
            raise ValueError(
                "At present cvxpy, must be installed for huber regression to be selected."
            )
...
class rvm(Solver):
    ...
    def solve(self, A, b):
        ...
        for i in range(self.max_iter):
            ...
            removed = np.append(
                removed, remaining_coeff_ind[np.where(gamma < 1e-10)[0]]
            )
            remaining_coeff_ind = np.setdiff1d(
                remaining_coeff_ind, remaining_coeff_ind[np.where(gamma < 1e-10)[0]]
            )
        ...
        if i == self.max_iter - 1:
            print("WARNING: Maximum iteration limit reached in solver.py")
...
class constrained_least_squares(Solver):
    ...
    def solve(self, A, b, C, d):
        ...
        if m >= n:
            self.coefficients = least_squares(
                {"verbose": self.verbose}
            ).get_coefficients(np.vstack([A, C]), np.vstack([b, d]))
        else:
            self.coefficients = constrained_least_squares._null_space_method(
                C, d, A, b, self.verbose
            )
...
class elastic_net(Solver):
    ...
    def __init__(self, solver_args={}):
        ...
        # Parse solver_args
        if "path" in self.solver_args:
            self.path = solver_args.get("path")
        if "alpha" in self.solver_args:
            self.alpha = solver_args.get("alpha")
        if "max_iter" in self.solver_args:
            self.max_iter = solver_args.get("max_iter")
        if "nlambdas" in self.solver_args:
            self.nlambdas = solver_args.get("nlambdas")
        if "lambda_eps" in self.solver_args:
            self.lambda_eps = solver_args.get("lambda_eps")
        if "lambda_max" in self.solver_args:
            self.lambda_max = solver_args.get("lambda_max")
        if "tol" in self.solver_args:
            self.tol = solver_args.get("tol")
        if "crit" in self.solver_args:
            self.crit = solver_args.get("crit")
    ...
    def solve(self, A, b):
        if self.path:
            ...
            self.coefficients = elastic_net._elastic_net_single(
                A, b, self.verbose, self.lamda, self.alpha, self.opt
            )
    ...
    def _elastic_net_single(A, b, verbose, lamda_val, alpha_val, opt):
        ...
        if opt == "osqp":
            ...
            objective = (
                0.5 * cv.sum_squares(A @ x - b)
                + lamda1 * cv.norm1(x)
                + 0.5 * lamda2 * cv.sum_squares(x)
            )
            prob = cv.Problem(cv.Minimize(objective))
            # Solve with OSQP
            lamda1.value = lamda_val * alpha_val
            lamda2.value = lamda_val * (1.0 - alpha_val)
        ...
        # Use scipy linprog for optimising
            ...
            raise ValueError(
                "At present cvxpy, must be installed for elastic net regression to be selected."
            )
        return x
    ...
    def _elastic_path(
        ...
        for fold in range(nfold):
            ...
            for l, lamda in enumerate(lamdas):
                ...
                x_path[l, :, fold] = elastic_net._elastic_net_cd(
                    x, A_train, b_train, lamda, alpha, max_iter, tol, verbose
                )
            ...
            rss[:, fold] = np.sum(
                (A_val @ x_path[:, :, fold].T - b_val.reshape(-1, 1)) ** 2, axis=0
            )
        ...
        if crit != "CV":
            df = np.count_nonzero(
                x_path[:, :, fold], axis=1
            )  # degrees of freedom can be approximated to be the number of non-zero coefficients [2]
        ...
        # "information criterion" is mean rss over nfold's for cross validation approach
            ...
            ic = np.nanmean(
                rss, axis=1
            )  # nanmean in case last fold had len(A_val) therefore that rss not defined
    ...
    def _elastic_net_cd(x, A, b, lamda, alpha, max_iter, tol, verbose):
        ...
        while not success:
            ...
            if attempt > 2:
                print(
                    "Non-zero coefficients still changing after two cycles, breaking..."
                )
                break
            ...
            for n_iter in range(max_iter):
                ...
                if (
                    n_iter == 0 or finish
                ):  # First iter or after convergence, loop through entire set
                    loop_set = set(range(p))
                elif (
                    n_iter == 1
                ):  # Now only loop through active set (i.e. non-zero coeffs)
                    loop_set = active_set
                ...
                for j in loop_set:
                    ...
                    if (
                        j == 0
                    ):  # TODO - check p0 is still at index 0 when more than parameter
                        x[j] = rho
                    else:
                ...
                if finish:
                    ...
                    if len(final_active_set - active_set) == 0:
                        ...
                        if verbose:
                            print(
                                "Final cycle added non-zero coefficients, restarting coordinate descent"
                            )
                    break
        return x
    ...
    def _get_lamdas(A, b, n_lamdas, lamda_eps, lamda_maxmax, alpha):
        ...
        if lamda_max <= np.finfo(float).resolution:
            ...
            return lamdas
        return np.logspace(
            np.log10(lamda_max * lamda_eps), np.log10(lamda_max), num=n_lamdas
        )[::-1]

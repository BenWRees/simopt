...
class Induced(Sampling):
    ...
    def _set_points(self, orders=None):
        ...
        sampled_cdf_values = np.random.rand(self.samples_number, self.dimensions)
        max_order = np.max(self.basis.orders)
        self.max_order = max_order
        quadrature_points = self._additive_mixture_sampling(
            index_set_used, sampled_cdf_values, max_order
        )
        if ray_imported:
            ray.shutdown()
        else:
    ...
    def _additive_mixture_sampling(self, index_set, cdf_values, max_order):
        ...
        for order in range(max_order + 1):
            ...
            inverse_cdf_values = self._univariate_sampling(
                parameter, sampled_cdf_values, order
            )
            if ray_imported:
    ...
    def inverse_induced_jacobi(self, alpha, beta, cdf_values, parameter):
        ...
        if order == 0:
            induced_points, induced_weights = parameter._get_local_quadrature(400 - 1)
            zeroes = np.array([])
            scaling_kn_factor = 0
        else:
            ...
            for root in zeroes:
                ...
                ab[0, 1] = 1
            induced_points, induced_weights = parameter._get_local_quadrature(
                400 - 1, ab
            )
            ...
            scaling_kn_factor = np.exp(-1.0 / order * np.sum(np.log(ab[:, 1])))
    ...
    def induced_jacobi_evaluation(
        ...
        for i in range(0, int(order)):
            ...
            logfactor += np.log(
                recurrence_ab[0, 1] * ((x + 1.0) / 2.0) ** 2 * scaling_kn_factor
            )
            recurrence_ab[0, 1] = 1
    ...
    def _linear_modification(self, ab, x0):
        ...
        r = np.reshape(
            np.abs(self._polynomial_ratios(alpha, beta, x0, N - 1)), (N - 1, 1)
        )
    ...
    def _quadratic_modification(self, alphabeta, x0):
        ...
        for i in range(0, N - 1):
            temp1[i] = (
                np.sqrt(beta[i + 1]) * C[i + 1] * C[i] * 1.0 / np.sqrt(1.0 + C[i] ** 2)
            )
        ...
        if N > 2:
            for nnn in range(2, N):
                C[nnn + 1] = (
                    ...
                    * (
                        (x - a[nnn]) * C[nnn]
                        - np.sqrt(b[nnn]) * C[nnn - 1] / np.sqrt(1.0 + C[nnn - 1] ** 2)
                    )
                )

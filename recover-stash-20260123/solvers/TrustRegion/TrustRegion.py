...
class TrustRegionBaseConfig(SolverConfig):
    """Configuration for TrustRegionBase solver."""

    crn_across_solns: Annotated[
        bool,
        Field(default=True, description="CRN across solutions?"),
    ]
    eta_1: Annotated[
        float,
        Field(
    ...
    sampling_rule: Annotated[
        str,
        Field(
            default="BasicSampling",
    ...
    model_construction_parameters: Annotated[
        dict,
        Field(
            ...
            alias="model construction parameters",
        ),
    ]
...
class TrustRegionBase(Solver):
    """Base class for Trust Region solvers."""

    config_class: ClassVar[type[SolverConfig]] = TrustRegionBaseConfig
    class_name_abbr: ClassVar[str] = "TRUSTREGION_BASE"
    class_name: ClassVar[str] = "TrustRegionBase"
...
class TrustRegionConfig(TrustRegionBaseConfig):
    """Configuration for TrustRegion solver."""

    pass  # Inherits all fields from TrustRegionBaseConfig


class TrustRegion(TrustRegionBase):
    """Trust Region solver."""

    config_class: ClassVar[type[SolverConfig]] = TrustRegionConfig
        ...
        if self.factors["easy_solve"]:
            ...
            if np.dot(np.multiply(grad, Hessian), grad) <= 0:
                ...
                tau = min(
                    1,
                    norm(grad) ** 3
                    / (delta * np.dot(np.multiply(grad, Hessian), grad)),
                )
    ...
    def evaluate_candidate_solution(
        ...
        stepsize = np.subtract(
            np.array(candidate_solution.x), np.array(current_solution.x)
        )
    ...
    def solve(self, problem: Problem) -> tuple[list[Solution], list[int]]:
        ...
        poly_basis_instance = self.polynomial_basis_instantiation()(
            self.factors["polynomial degree"], problem, None, problem.dim
        )
        ...
        while self.budget.remaining > 0:
            ...
            if k == 1 and hasattr(sampling_instance.__class__, "calculate_kappa"):
                ...
                current_solution = sampling_instance.calculate_kappa(
                    k, problem, current_solution, delta_k
                )

                self.recommended_solns.append(current_solution)
                self.intermediate_budgets.append(self.budget.used)
                self.fn_estimates.append(current_solution.objectives_mean.item())
                self.iterations.append(k)
                self.budget_history.append(0)
            elif self.factors["crn_across_solns"] and hasattr(
                sampling_instance.__class__, "calculate_kappa"
            ):
                ...
                while True:
                    ...
                    stopping = sampling_instance.get_stopping_time(
                        sig2, delta_k, k, problem
                    )
                    if (
            ...
            # build random model
            current_solution, delta_k, interpolation_solns, visited_pts_list = (
                model.construct_model(current_solution, delta_k, k, visited_pts_list)
            )
            # expended_budget = construction_budget # the additions to the expended budget is done in model.construct_model

            # solve random model
            candidate_solution, visited_pts_list = self.solve_subproblem(
                delta_k, model, problem, current_solution, visited_pts_list
            )

            candidate_solution, fval_tilde = self.sample_candidate_solution(
                sampling_instance,
                candidate_solution,
                current_solution,
                problem,
            ...
            current_solution, delta_k = self.evaluate_candidate_solution(
                model,
                problem,
                fval_tilde,
                delta_k,

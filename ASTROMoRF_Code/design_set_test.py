...
def interpolation_points_without_reuse(current_solution: Solution, problem: Problem, U: np.ndarray, delta: float) -> list[np.ndarray]:
	"""
...
def interpolation_points_with_reuse(current_solution: Solution, problem: Problem, reused_x: np.ndarray, rotation_vectors: list[np.ndarray], U: np.ndarray, delta: float) -> list[np.ndarray]:
	"""
...
def construct_interpolation_set(k: int, current_solution: Solution, problem: Problem, U: np.ndarray, delta: float, visited_pts: list[Solution]) -> tuple[list[np.ndarray], int] : 
		"""
		...
		if (k == 1) or norm(x_k - np.array(visited_pts[f_index].x).reshape(-1,1))==0 :
			Y = interpolation_points_without_reuse(current_solution, problem, U, delta)
		...
		# Else if we will reuse one design point
			...
			Y = interpolation_points_with_reuse(current_solution, problem, reused_pt, rotate_matrix, U, delta)

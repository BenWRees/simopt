...
def sample_set(k: int, problem: Problem, current_solution: Solution, delta_k: float, current_f: float, U: np.ndarray,
    ...
    """
    Improves the design set by filtering existing points and generating new candidates within
    ...
    if S_hat.shape[0] > 0 and max(norm(S_hat - s_old.ravel(), axis=1, ord=np.inf)) > dist:
    ...
    X_additional = generate_set(problem, num_additional, current_solution, delta_k)
    X_additional, _ = remove_point_from_set(X_additional, np.zeros((X_additional.shape[0], 1)), s_old)
    ...
    if X_additional.shape[0] > 0:
        S_hat = np.vstack([S_hat, X_additional])
    ...
    X_new, fX_new, delta_k, visited_pts, interpolation_solutions, expended_budget = LU_pivoting(
        k, problem, X_new, fX_new, current_solution, delta_k, S_hat, f_hat, U, 
        expended_budget, visited_pts, interpolation_solutions
    )
    
    return X_new, fX_new, delta_k, visited_pts, interpolation_solutions, expended_budget

def LU_pivoting(k: int, problem: Problem, X: np.ndarray, fX: np.ndarray, current_solution: Solution, delta: float, X_improved: np.ndarray, fX_improved: np.ndarray, U: np.ndarray,
    ...
    phi_function, phi_function_deriv, delta = get_phi_function_and_derivative(X_improved, current_solution, delta, U)
    ...
    for k in range(1, max_k):  # Build at most q points for the LU steps
        ...
        if X_improved is not None and X_improved.size > 0:
            Phi_X_improved = np.vstack([phi_function(X_improved[i, :].reshape(-1, 1)).ravel() for i in range(X_improved.shape[0])])
            M = np.abs(Phi_X_improved @ v)
            index = int(np.argmax(M))
            # pivot thresholds
            if M[index] < psi_1:
                flag = False
            elif (k == max_k - 1 and M[index] < psi_2):  # last pivot check (relative to q)
                flag = False
        else:
            flag = False
        ...
        if flag and index is not None:
            ...
            x = None
            try:
                x_candidate = find_new_point(problem, delta, v, phi_function, phi_function_deriv)
                ...
                try:
                    x_candidate = find_new_point_alternative(problem, current_solution, v, phi_function, X[:k, :], delta, U)
                    x_proj = U.T @ x_candidate.reshape(-1, 1)
                    if norm(x_proj - x_projected_k, ord=2) <= delta:
                        x = x_candidate
                except Exception:
            ...
            if (X_improved is not None and X_improved.size > 0 and index is not None and M is not None):
                try:
        ...
        if abs(R[k, k]) < min_pivot:
            ...
            try:
                s_backup = find_new_point_alternative(problem, current_solution, v, phi_function, X[:k, :], delta, U)
...
def find_new_point_alternative(problem: Problem, current_solution: Solution, v: np.ndarray, phi_function: callable, S: np.ndarray, delta_k: float, U: np.ndarray = None) -> np.ndarray:
    s_old = np.array(current_solution.x).reshape(-1,1)

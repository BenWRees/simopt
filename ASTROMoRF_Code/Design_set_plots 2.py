"""
This script creates three plots showing the evolution of PROJECTED design sets in ASTRO-MoRF:
Plots show the projected design set X @ U being well-poised on the projected trust region
...
def compute_poisedness(X, center, delta, U):
    """
    ...
    Y = np.array([U.T @ (X[i, :].reshape(-1, 1) - s_old) / delta for i in range(n_points)])
    Y = Y.reshape(n_points, d)
    
    # Build polynomial basis matrix (quadratic)
    if d == 2:
        Phi = np.zeros((n_points, 6))
        for i in range(n_points):
            y1, y2 = Y[i, 0], Y[i, 1]
            Phi[i, :] = [1, y1, y2, y1**2, y1*y2, y2**2]
    else:
        # General case for arbitrary d
        Phi = np.ones((n_points, 1))
        for i in range(n_points):
            y = Y[i, :]
            Phi = np.hstack([Phi, y.reshape(1, -1)])
...
def plot_design_set(ax, design_points, center, radius, U, title, reused_point=None):
    """
    ...
    if U.shape[1] == 2:
        ...
        circle = Circle(projected_center, radius, fill=False, color='blue', 
                       linestyle='-', linewidth=2, label='Trust Region')
        ax.add_patch(circle)
        
        # Plot the projected center point
        ax.plot(projected_center[0], projected_center[1], 'ko', markersize=10, 
               zorder=5, label='Center')
        
        # Plot projected design points
        ax.plot(projected_points[:, 0], projected_points[:, 1], 'go', markersize=8, 
               zorder=3, label='Design Points')
        ...
        if reused_point is not None:
            ...
            ax.plot(projected_reused[0], projected_reused[1], 'ms', markersize=12, 
                   zorder=4, label='Reused Point')
        ...
        margin = radius * 0.5
        ax.set_xlim(projected_center[0] - radius - margin, 
                   projected_center[0] + radius + margin)
        ax.set_ylim(projected_center[1] - radius - margin, 
                   projected_center[1] + radius + margin)
        ...
        ax.axvline(x=projected_center[0], color='blue', linestyle='-', 
                  linewidth=2, label='Center')
        ax.axvline(x=projected_center[0] - radius, color='blue', linestyle='--', 
        ...
        ax.plot(projected_points[:, 0], y_coords, 'go', markersize=8, 
               zorder=3, label='Design Points')
        
        if reused_point is not None:
            reused_point = np.array(reused_point).ravel()
            projected_reused = (reused_point - center) @ U
            ax.plot(projected_reused[0], 0, 'ms', markersize=12, 
                   zorder=4, label='Reused Point')
        ...
        raise ValueError(f"Cannot plot {U.shape[1]}D active subspace. Only 1D or 2D supported.")
...
def main():
    ...
    if full_dim == 2:
        ...
        U = np.eye(full_dim)[:, :2]
        print(f"Using first 2 columns of identity as active subspace")
    ...
    print(f"Projected points plotted in active subspace")
    ...
    X_before_improvement = design_set_2.copy()
    ...
    fn = lambda x: np.linalg.norm(x - np.array(new_solution.x)) ** 2
    fX_before = np.array([fn(X_before_improvement[i, :]) for i in range(X_before_improvement.shape[0])]).reshape(-1, 1)
    ...
    cond_before, det_before = compute_poisedness(X_before_improvement, new_solution.x, radius, U)
    ...
    X_improved, fX_improved, interpolation_sols, visited_pts_list, updated_radius, _ = improve_geometry(
        k=2,
        problem=problem,
        current_solution=new_solution,
    ...
    print(f"Design sets are different: {not np.allclose(X_improved, X_before_improvement)}")
    
    # Compute poisedness AFTER improvement (using reduced radius, in projected space)
    cond_after, det_after = compute_poisedness(X_improved, new_solution.x, reduced_radius, U)
    ...
    # Compare poisedness
    print(f"\n{'='*60}")
    print(f"POISEDNESS COMPARISON (in projected active subspace):")
    print(f"{'='*60}")
    ...
    if cond_after < cond_before:
        ...
        print(f"✗ Condition number got worse")
    ...
    if det_before is not None and det_after is not None:
        ...
        if det_after > det_before:
            print(f"✓ Determinant IMPROVED (larger absolute value)")
        else:
            print(f"✗ Determinant got smaller")
    print(f"{'='*60}\n")
    ...
    fn_no_reuse = lambda x: np.linalg.norm(x - center_no_reuse) ** 2
    fX_no_reuse = np.array([fn_no_reuse(design_set_no_reuse[i, :]) for i in range(design_set_no_reuse.shape[0])]).reshape(-1, 1)
    interpolation_sols_no_reuse = [Solution(tuple(x), problem) for x in design_set_no_reuse]
    reduced_radius_no_reuse = radius

    # Improve geometry on the fresh design set (visited list empty => no reuse)
    X_improved_no_reuse, fX_improved_no_reuse, interpolation_sols_no_reuse, visited_pts_list_no_reuse, updated_radius_no_reuse, _ = improve_geometry(
        k=2,
        problem=problem,
        current_solution=new_solution,
    ...
    print(f"Projected design set (no reuse) BEFORE:\n{(design_set_no_reuse - center_no_reuse) @ U}")
    ...
    cond_no_reuse_before, det_no_reuse_before = compute_poisedness(design_set_no_reuse, new_solution.x, radius, U)
    cond_no_reuse_after, det_no_reuse_after = compute_poisedness(X_improved_no_reuse, new_solution.x, reduced_radius_no_reuse, U)
    ...
    print(f"Poisedness AFTER improvement (no reuse, projected):")
    ...
    plt.tight_layout()
    plt.savefig('design_set_evolution_projected_4plots.png', dpi=300, bbox_inches='tight')
    print("\n✓ Plots saved as 'design_set_evolution_projected_4plots.png'")
    print(f"✓ Fourth plot shows geometry improvement with NO reused points.")
    plt.show()

"""This plot shows how the mean squared error of the constructed model changes as point.

sampled move away from the center point.
"""
...
from geometry_improvement_test import generate_set
from Model_construction import fit
...
def adaptive_radius_plot(problem_name, initial_radius) -> None:  # noqa: ANN001
    """Plots the mean squared error of the constructed model as a function of the radius.  # noqa: E501.
    ...
    f_curr, init_solutions = calculate_responses(
        np.array([initial_solution_array]).reshape(1, -1), problem
    )
    ...
    init_S_full = generate_set(problem, dim, initial_solution, initial_radius)  # noqa: N806
    U0, _ = np.linalg.qr(init_S_full.T)  # noqa: N806
    ...
    for radius in radii:
        ...
        (
            ...
            visited_pts,
            radius,
        ) = construct_design_set(
            initial_solution, degree, problem, f_curr, U0, radius, visited_pts
        )

        # Construct model
        _U, model, _model_grad, X, _fX, interpolation_solutions, visited_pts, _delta = (  # noqa: N806
            fit(
                k,
                degree,
                problem,
        ...
        r_squared_val = calculate_r_squared(
            initial_radius, problem, new_solution, model
        )
    ...
    plt.fill_betweenx(
        [min(r_squared) - 2, max(r_squared) + 2],
        0.7,
        0.8,
        color="orange",
        alpha=0.45,
        label="Typical Radius Range",
    )
...
def construct_design_set(  # noqa: ANN201
    ...
    """Construct the design set for model building. Ensures we are testing the reuse.

    design points construction.


    by first constructing a design set without reuse, then moving the current solution,
    and constructing
    a design set with reuse.
    """
    # First iteration of design set construction
    X, _ = construct_interpolation_set(  # noqa: N806
        1, current_solution, problem, U, radius, visited_pts
    )
    f_values, interpolation_solutions = calculate_responses(X, problem)

    U, model, model_grad, X, _fX, interpolation_solutions, visited_pts, _delta = fit(  # noqa: N806
        1,
        degree,
        problem,
        X,
    ...
    new_solution = solve_subproblem(
        current_solution, model, model_grad, problem, U, radius
    )

    new_fval, new_solution = calculate_responses(
        np.array([new_solution.x]).reshape(1, -1), problem
    )
    ...
    X, _ = construct_interpolation_set(2, new_solution, problem, U, radius, visited_pts)  # noqa: N806
    f_values, interpolation_solutions = calculate_responses(X, problem)
...
def generate_test_points_in_ball(
    center: np.ndarray, radius: float, num_points: int, n: int
) -> np.ndarray:
    ...
    def random_pt():  # noqa: ANN202
        ...
        return center + distance * direction  # Scale and shift to center
...
def calculate_r_squared(
    ...
    """Calculate the mean squared error of the model within the given radius around the.

    center.
    """
    # randomly generate no_pts test points within the ball of radius radius and center current_solution.x
    test_points = generate_test_points_in_ball(
        np.array(current_solution.x), radius, no_pts, problem.dim
    )
    ...
    1 - (ss_res / ss_tot) if ss_tot > 1e-12 else 0.0
...
def calculate_responses(X: np.ndarray, problem: Problem):  # noqa: ANN201, N803
    ...
    # get responses
    responses = np.array(
        [-1 * problem.minmax[0] * sol.objectives_mean.item() for sol in simulated_sols]
    ).reshape(-1, 1)  # shape (M, )
    return responses, simulated_sols


def solve_subproblem(  # noqa: D417
    current_solution: Solution,
    model: callable,
    model_grad: callable,
    problem: Problem,
    ...
    regularization.
        ...
        The regularization term penalizes small full-space steps for given reduced-space
        steps,
        encouraging solutions that make substantial progress in full space.

        NEW APPROACH: Penalize based on ratio of norms ||U·s|| / ||s|| to encourage
        large full-space steps
        while keeping the optimization stable. The norm is L-1 for better numerical
        behavior.
    ...
    def obj_fn(z):  # noqa: ANN001, ANN202
        ...
        return float(model_val + lambda_reg * ratio_penalty)
    ...
    def cons_fn(z):  # noqa: ANN001, ANN202
    ...
    if step_norm_reduced < boundary_tolerance:  # noqa: SIM102

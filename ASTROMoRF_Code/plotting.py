...
sys.path.append(str(Path(sys.modules[__name__].__file__).parent.parent.resolve()))  # type:ignore
...
import numpy as np
from Model_construction import fit_coef, model_evaluate
...
def model_callable(
    ...
    def _model(X: np.ndarray) -> np.ndarray:  # noqa: N803
        predictions = []
        for x in X:
...
def get_evals_of_problem(problem: Problem, X: np.ndarray) -> np.ndarray:  # noqa: N803
...
def plot_1d_model_construction(
    ...
    true_y = np.asarray(get_evals_of_problem(objective, grid_x.reshape(-1, 1))).reshape(
        -1
    )
...
def plot_2d_model_construction(  # noqa: D417
    ...
    if not callable(model):
        model = model_callable_from_q(np.asarray(model), dim=2)  # noqa: F821

    xx = np.linspace(x_range[0], x_range[1], n_grid)
    yy = np.linspace(y_range[0], y_range[1], n_grid)
    Xg, Yg = np.meshgrid(xx, yy)  # noqa: N806
    ...
    c = ax.contour(
        Xg, Yg, model_vals, levels=levels // 2, colors="white", linewidths=1, alpha=0.9
    )

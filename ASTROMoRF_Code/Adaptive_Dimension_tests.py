"""
    Within this script, we test ASTROMoRF's adaptive dimension mechanic and see if it accurately predicts the optimal dimension within a trust-region
"""
...
def detect_plateau() -> bool:
    ...
    no_improvement = all(abs(recent_values[i] - recent_values[0]) < improvement_threshold for i in range(1, len(recent_values)))

    is_plateau = (
        (relative_improvement < improvement_threshold and unsucusseful_iteration_count >= 3) 
...
def estimate_from_eigenvalues() -> int:
    pass 


#! ==== Run iterations ====
def run_iterations(num_iterations: int) -> None:
    pass 
